/**
 * Czech dialog translations for BurningBearStreetScene
 * Speaker: Ortolan (Day 2 — Infinite Fold)
 */
export default {
    _speakers: {
        'Ortolan': 'Ortolan',
        'Priest': 'Kněz',
    },
    ortolan_bb_start: {
        text: {
            asked: "Nezvedne oči od účetní knihy, kterou balancuje na dvou rukou. \"Zase vy. Sklep pod Scraperem — tam ta stará verze bude, jestli je vůbec někde. A teď mě nechte vrátit se k formulářům.\"",
            helped: "\"No — jestlipak to není ten, kdo mi z té papírové mašinérie vydupal druhý pár rukou.\" Zahýbe všemi čtyřma, spokojeně. \"Laskavost nezapomínám. Ale tváříte se jako někdo, koho posílá Elphi... a my dva jsme spolu roky slušně nepromluvili. Co chce?\"",
            cold: "Sotva vzhlédne od formulářů. \"Vy. Díval jste se, jak se topím v povoleních, a šel jste dál. A teď se tu objevíte a oháníte se Elphiným jménem jako odznakem. Roky spolu nemluvíme, ona a já. Řekněte, co chcete, a nechte mě pracovat.\""
        },
        options: {
            perspective_ortolan_open: "[Než vejdu do katedrály] Ty jsi ty staré systémy stavěl. Do čeho to jdu?",
            right_scraper_cellar: "Jasně. Sklep pod Scraperem.",
            the_infinite_loop_running: "Infinite Fold. Elphi říká, že zase běží.",
            never_mind_bb: "Nic, nevadí."
        }
    },
    ortolan_bb_perspective: {
        text: "Odloží účetní knihu, co má nejblíž, a poprvně ti věnuje pozornost všech čtyř rukou. \"Tak tys byl dole ve sklepě. Vidím to na tobě — ten pohled někoho, komu stroj odpověděl.\n\nTak si tedy poslechni upřímné slovo od tvůrce, protože Elphi ti ho nikdy neřekne: něco může být krásné a přesto špatně navržené. Postavil jsem obojí, někdy i v jednom odpoledni. Věc může zpívat a přitom být od kořene rozbitá — ta vada není opakem té krásy, ona je do ní *zapečená*. To, co se líhne v katedrále, není jiné. Je to starší výrobek než cokoli, co jsem kdy sešrouboval, ale pořád to bylo *uděláno*, a všechno udělané má švy. Jestli tam jdeš, jdi s tím vědomím.\"",
        options: {
            ortolan_bb_perspective_machinery: "Co to obklopuje? Ta stará mašinerie.",
            ortolan_bb_perspective_guard: "Pověz mi o tom strážci u dveří.",
            ortolan_bb_perspective_close: "Budu na to pamatovat."
        }
    },
    ortolan_bb_mechanisms: {
        text: "\"Ta mašinerie? Práce starého řádu, celá — předválečná, z doby, než vejce vůbec prorazilo zem. Regulátory, tlumiče, nosné ochrany; přesně to, co stavíš, aby to udrželo místo *v klidu*, zatímco se uvnitř peče něco křehkého.\" Zaťuká kloubem o stůl v pomalém, mechanickém rytmu. \"Většina z toho teď selhává. Ne dramaticky — v tom je ta past. Selhává to tak, jak selhává starý kód: tiše, po koutech, dělá špatnou věc s naprostou jistotou. Muži, kteří to ladili, jsou desítky let mrtví, a to místo rostlo kolem jejich nastavení jako strom, co pohltí plot.\n\nTak nevěř ničemu tam uvnitř, co si hučí, jako by vědělo, co dělá. To vědění-co-dělám je přesně ten způsob selhání. Dobře udělaná věc, ponechaná běžet dost dlouho, přestane být nástrojem a začne být *počasím*.\"",
        options: {
            ortolan_bb_mechanisms_guard: "A ten strážce u dveří?",
            ortolan_bb_mechanisms_back: "Zeptat se na něco jiného.",
            ortolan_bb_mechanisms_close: "To stačí. Děkuji, Ortolane."
        }
    },
    ortolan_bb_sentinel: {
        text: "\"Strážce Závoje.\" Vysloví to jméno tak, jak bys jmenoval stroj, který ctíš a tak úplně mu nevěříš. \"Tenhle je poslední z nich — poslední mechanismus starého řádu, co ještě poctivě běží. Maso proměněné v rostlinu, mysl vpletená do městské myceliální sítě; ve skutečnosti strážní systém, převlečený za strážce. Kdosi mu před dávnými časy dal jediný příkaz — *drž závoj zavřený* — a každý, kdo ten příkaz vydal, je teď mrtvý, a on ho pořád vykonává. Věrně. Bez odvolání.\n\nProto je nebezpečný, a je to táž vada jako u té věci ve sklepě: udělaná věc, plnící svůj účel dál i po smrti všech, kdo jí mohli říct, ať přestane. Nebude uvažovat. Nepovolí. Zná jen pravidlo, které dostal. Jestli chceš kolem něj projít, nehádej se s tím mužem — žádný muž tam už není. Pracuj s tím pravidlem.\"",
        options: {
            ortolan_bb_sentinel_machinery: "Jak s tím souvisí okolní systémy?",
            ortolan_bb_sentinel_back: "Zeptat se na něco jiného.",
            ortolan_bb_sentinel_close: "To stačí. Děkuji, Ortolane."
        }
    },
    ortolan_bb_loop: {
        text: "\"Infinite Fold.\" Řekne to ploše — spíš otráveně než vyděšeně. \"Starý experiment. Postavili jsme ho s Elphi, před lety: ona tvarovala sen, já hru pod ním. Nikdy jsme to nedokončili. Vládci města znervózněli — nazvali to nebezpečným, *nepředvídatelným* — a celý projekt zastavili.\n\nPotom jsme se s Elphi pohádali o to, kdo si nechá klíče od těch her. Od té doby nemluvíme.\" Pokrčí rameny. \"Byla to hra. Nic víc to nikdy nebylo.\"",
        options: {
            elphi_thinks_it_killed: "Elphi si myslí, že to zabilo Biskupku.",
            still_a_copy_anywhere: "Existuje ještě někde kopie?"
        }
    },
    ortolan_bb_danger: {
        text: "Vážně se zasměje. \"Že ji to zabilo? Elphi se vždycky lekala vlastního stínu. Hra nesáhne z helmy ven a nezastaví srdce. Jestli Biskupka zemřela, když to běželo, hledejte chybu v hardwaru — nebo v ruce, co tu kazetu upravila — ne v mých pravidlech.\n\nInfinite Fold je náladový. Ne vražedný.\" Přeběhne přes něj záblesk pochyby, hned zase pohřbený. \"...Nedodělaná, budiž. Ale tohle ne.\"",
        options: {
            where_could_a_copy_be: "Tak kde by mohla být kopie?",
            i_hope_youre_right: "Doufám, že máte pravdu."
        }
    },
    ortolan_bb_copy: {
        text: {
            helped: "\"Kopie?\" Odloží účetní knihu a znehybní — všechny čtyři ruce v klidu. \"Master verze. Bude pořád ve sklepě pod Scraperem, kde bývala naše stará laboratoř a pracovna. Zapečetěná, založená a zapomenutá, jako všechno, čeho se vládci bojí.\n\nA sklep je zamčený — klíč už nemám, vládci si ho vzali. Ale heslo pro výtah... to jsme si nevymysleli. Použili jsme pravé jméno jednoho mrtvého boha — takové, které si pamatuje už jen jeho náhrobek. Přečti si hroby na Bohohřbitově pod Radnicí; divinograf tě provede. To pravé poznáš.\n\nA protože jsi mi prokázal laskavost — povím ti, k čemu to doopravdy bylo. Infinite Fold jsme tomu říkali. Nikdy to nebyla hra, kterou bys mohl vyhrát nebo prohrát; nebyl v ní vítězný tah ani prohrávající. Byla to otázka, kvůli které jsme postavili celý stroj: může vzniknout myšlenka, která nemá jediného autora? Nechali jsme tisíce hráčů, aby do toho vlévali své drobné volby a sny, a sledovali jsme, jestli z nich všech naráz vyroste něco nového — způsob významu, který nikdo z nás nenapsal. Vládci to zarazili právě proto, že to začalo fungovat: začalo to dávat smysl způsoby, které jsme si neuměli vysvětlit. Tak se mě neptej, co je ta stará zapečetěná verze teď. Ptej se, čím se to mělo stát. Ta otázka byla celý ten experiment — a oni se odpovědi báli dost na to, aby to pohřbili tam dole.\"",
            cold: "\"Kopie.\" Ledabyle se prohrabává formulářem. \"Ve sklepě pod Scraperem bude stará verze, tam byla laboratoř. Je zapečetěný — výtah chce heslo, které jsem dávno zapomněl. Něco s tím hřbitovem pod Radnicí, jestli ti to něco říká. Dál už si poraď sám. Mám vlastní papírování a s tím mým jsi mi zrovna nepomohl.\" Otočí se zpátky ke svým formulářům."
        },
        options: {
            thanks_ortolan_bb: "Děkuji, Ortolane."
        }
    },

    // ===== Gang of Lamps (Torchèrův úkol): schránka pro pašované zboží =====
    smuggle_drop: {
        text: `Mříž je rzí přivařená ke kočičím hlavám a Torchèrův balíček těžce spočívá v kabátě — Wimlick, taková váha, o jaké se celníkům zdá. Ulice je tichá. Prozatím.`,
        options: {
            smuggle_plain: "Vypáčit mříž a zastrčit ho dovnitř.",
            smuggle_ulvarex: "[Ulvarex · Tkaní přeludů] Zahalit balíček a upustit ho nepozorovaně.",
            smuggle_palinode: "[Palinode] Odeříct zrezivělou mříž, aby se otevřela jako nová.",
            smuggle_keep: "[Zrada] Nechat si ho. Torchèrovi říct, že je hotovo. (Co s ním udělám, je moje věc.)",
            smuggle_cancel: "Tady ne. Ještě ne.",
        }
    }
};
