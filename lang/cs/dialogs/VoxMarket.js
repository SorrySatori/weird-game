/**
 * Czech dialog translations for VoxMarket
 * Speaker: Kloor Venn
 */
export default {
    _speakers: {
        'Kloor Venn': 'Kloor Venn',
    },
    kloor_start: {
        text: "Kloor Venn si tě prohlíží s podezřením i zájmem. 'Co chceš? Mám práci.'",
        options: {
            who_are_you: "Kdo jste?",
            what_are_you_selling: "Co prodáváte?",
            i_want_to_sell_you_some_spores: "Chci vám prodat nějaké spory.",
            do_you_know_anything_about_the_bishop: "Víte něco o Biskupce?",
            about_those_vestigels: "O těch Vestigelech...",
            can_you_help_me_to_get_to_rust_choir_headquarters: "Můžete mi pomoct dostat se do centrály Rezavého chóru?",
            i_need_your_expertise_on_some_sulkberries: "Potřebuji vaši expertízu ohledně smutnic."
        }
    },
    kloor_who: {
        text: "'Jmenuji se Kloor Venn. Jsem... farmaceutický podnikatel. Obchoduji se specializovanými látkami, které rozšiřují mysl.' Poklepe si na spánek a ušklíbne se.",
        options: {
            what_kind_of_substances: "Jaké látky?",
            back: "Zpět"
        }
    },
    kloor_substances: {
        text: "'Specializuji se na Oltrac — vzácnou psychoaktivní sloučeninu získávanou z určitých... biologických materiálů.' Se zájmem si prohlíží tvé houbové výrůstky. 'Materiálů nepodobných tomu, co zjevně nosíš s sebou.'",
        options: {
            tell_me_more_about_oltrac: "Řekněte mi víc o Oltracu",
            back: "Zpět"
        }
    },
    kloor_oltrac: {
        text: "'Oltrac existuje v různých variantách. Šedý je běžný, Fialový je silnější a Jantarový... no, Jantarový Oltrac je něco speciálního. Otevírá dveře v mysli, o kterých většina ani neví. Kvalita závisí na zdrojovém materiálu.'",
        options: {
            i_want_to_buy_some: "Chci něco koupit",
            i_could_sell_you_some_spores: "Mohl bych vám prodat nějaké spory",
            back: "Zpět"
        }
    },
    kloor_selling: {
        text: "'Obchoduji s Oltracem — nejlepší látkou na rozšíření mysli na celém Voxmarketu. Otevírá tvé vnímání pravé podstatě reality.' Ztlumí hlas. 'Máte zájem o koupi? Nebo snad... prodat mi některé z těch spor, co nosíte?'",
        options: {
            show_me_what_you_have: "Ukažte, co máte",
            i_could_sell_you_some_spores: "Mohl bych vám prodat nějaké spory",
            back: "Zpět"
        }
    },
    kloor_shop: {
        text: "'Prohlédněte si mé zboží. Kvalita zaručena.'",
        options: {
            not_now: "Teď ne"
        }
    },
    kloor_buy_spores: {
        text: "Kloorovi se rozzáří oči zájmem. 'Vždy mám zájem o kvalitní spory. Jsou klíčovou ingrediencí mého Oltracu. Zaplatím vám podle toho, co z nich můžu vyrobit. Kolik jich jste ochotni postrádat?'",
        options: {
            sell_10_spores: "Prodat 10 spor",
            sell_20_spores: "Prodat 20 spor",
            sell_30_spores: "Prodat 30 spor",
            not_now: "Teď ne"
        }
    },
    kloor_sell_spores_10: {
        text: "Nabízíte Kloorovi prodat 10 spor.",
        options: {
            confirm: "Potvrdit",
            back: "Zpět"
        }
    },
    kloor_sell_spores_20: {
        text: "Nabízíte Kloorovi prodat 20 spor.",
        options: {
            confirm: "Potvrdit",
            back: "Zpět"
        }
    },
    kloor_sell_spores_30: {
        text: "Nabízíte Kloorovi prodat 30 spor.",
        options: {
            confirm: "Potvrdit",
            back: "Zpět"
        }
    },
    kloor_not_enough_spores: {
        text: "Kloor se zamračí. 'Nemáte dost spor. Přijďte, až nasbíráte víc.'",
        options: {
            ok: "OK"
        }
    },
    kloor_gray_oltrac_8: {
        text: "Kloor pečlivě prozkoumá vaše spory a pak přikývne. 'Tyhle poslouží na Šedý Oltrac — běžné zboží. Ujde to.' Podá vám 8 zlatých mincí. 'Rádo se obchoduje.'",
        options: { "Thanks": "Díky" }
    },
    kloor_gray_oltrac_16: {
        text: "Kloor pečlivě prozkoumá vaše spory a pak přikývne. 'Tyhle poslouží na Šedý Oltrac — běžné zboží. Ujde to.' Podá vám 16 zlatých mincí. 'Rádo se obchoduje.'",
        options: { "Thanks": "Díky" }
    },
    kloor_gray_oltrac_24: {
        text: "Kloor pečlivě prozkoumá vaše spory a pak přikývne. 'Tyhle poslouží na Šedý Oltrac — běžné zboží. Ujde to.' Podá vám 24 zlatých mincí. 'Rádo se obchoduje.'",
        options: { "Thanks": "Díky" }
    },
    kloor_violet_oltrac_15: {
        text: "Kloorovi se rozzáří oči, když prozkoumá vaše spory. 'Vynikající kvalita! Z těchto můžu vyrobit Fialový Oltrac.' S úšklebkem vám podá 15 zlatých mincí. 'Opravdu dobrý obchod.'",
        options: { "Thanks": "Díky" }
    },
    kloor_violet_oltrac_30: {
        text: "Kloorovi se rozzáří oči, když prozkoumá vaše spory. 'Vynikající kvalita! Z těchto můžu vyrobit Fialový Oltrac.' S úšklebkem vám podá 30 zlatých mincí. 'Opravdu dobrý obchod.'",
        options: { "Thanks": "Díky" }
    },
    kloor_violet_oltrac_45: {
        text: "Kloorovi se rozzáří oči, když prozkoumá vaše spory. 'Vynikající kvalita! Z těchto můžu vyrobit Fialový Oltrac.' S úšklebkem vám podá 45 zlatých mincí. 'Opravdu dobrý obchod.'",
        options: { "Thanks": "Díky" }
    },
    kloor_amber_oltrac_25: {
        text: "Kloor při zkoumání vašich spor zalapá po dechu. 'Mimořádné! Tyhle jsou perfektní na Jantarový Oltrac — nejkvalitnější druh!' Dychtivě vám napočítá 25 zlatých mincí. 'Výjimečný obchod! Přijďte kdykoliv!'",
        options: { "Thanks": "Díky" }
    },
    kloor_amber_oltrac_50: {
        text: "Kloor při zkoumání vašich spor zalapá po dechu. 'Mimořádné! Tyhle jsou perfektní na Jantarový Oltrac — nejkvalitnější druh!' Dychtivě vám napočítá 50 zlatých mincí. 'Výjimečný obchod! Přijďte kdykoliv!'",
        options: { "Thanks": "Díky" }
    },
    kloor_amber_oltrac_75: {
        text: "Kloor při zkoumání vašich spor zalapá po dechu. 'Mimořádné! Tyhle jsou perfektní na Jantarový Oltrac — nejkvalitnější druh!' Dychtivě vám napočítá 75 zlatých mincí. 'Výjimečný obchod! Přijďte kdykoliv!'",
        options: { "Thanks": "Díky" }
    },
    kloor_bishop: {
        text: "Kloorův výraz se změní na opatrný. 'Biskupka? Jo, viděl jsem ji tady. S tou si není radno zahrávat. Byla tu na trhu nedávno, obchodovala s některými kupci.'",
        options: {
            do_you_know_where_she_went: "Víte, kam šla?",
            what_was_she_trading: "S čím obchodovala?",
            back: "Zpět"
        }
    },
    kloor_sulkberry_analyze: {
        text: "Kloor převezme vzorek smutnice cvičenými prsty, otáčí jí mezi palcem a ukazováčkem. Přičichne, pak z kabátu vytáhne malou skleněnou nádobu a rozdrítí do ní úlomek. Kapalina se zbarví do hlubokého, čistého jantaru.\n\n'Kořeněná smutnice. Prémiovâ kvalita — toto je zboží direktoriátu, bez pochyb. Alkaloidní profil je...' Zvedne nádobu proti světlu. '...učebnicový. '\n\nOdloží nádobu. 'S tímhle plodem není nic špatně. Žádné příměsi, žádné toxiny, žádné cizí látky.'",
        options: {
            photosentience_read_kloors_biosignals_for_deceptio: "[Fotosenzitivita] Číst Kloorovy biosignály kvůli podvodu.",
            youre_sure_your_reputation_is_on_the_line: "Jste si jistý? Je to o vaši reputaci.",
            thanks_for_the_analysis: "Díky za analýzu."
        }
    },
    kloor_sulkberry_neme: {
        text: "Necháte Nemovo vědomí prosáknout ven a číst síť biosignálů kolem Kloorovy nervové soustavy. Zájem — upřímný, vědecký zájem. Profesní hrdost z jeho analýzy. Záblesk oportunismu, přemýšlející, jestli by vám nemohl účtovat za konzultaci. Ale žádný podvod. Žádné zatajování.\n\nNeme mrumlá: 'Obchodník s drogami je spousta věcí, ale lhář ohledně chemie on není. Jeho marné ego by mu to nedovolilo.'\n\nKloorova analýza je upřímná. Smutnice jsou čisté.",
        options: {
            good_to_know_thanks_kloor: "Dobré vědět. Díky, Kloore."
        }
    },
    kloor_sulkberry_sure: {
        text: "'Moje reputace JE můj živobytí.' Kloor vypadá téměř uraženě. 'Analyzuji alkaloidní sloučeniny celé roky. Vím, jak vypadá kontaminace — ať už podle barvy nebo krystalické struktury.\n\nTyhle plody jsou nedotčené.'",
        options: {
            back: "Zpět"
        }
    },
    kloor_bishop_location: {
        text: "'Nedokážu říct s jistotou. Biskupka kráčí záhadnými cestami.' Ušklíbne se vlastnímu vtipu. 'Ale slyšel jsem, že směřovala k Hale. Má tam kontakty.'",
        options: {
            thanks_for_the_information: "Díky za informaci",
            what_was_she_trading: "S čím obchodovala?"
        }
    },
    kloor_bishop_trading: {
        text: "'Teď to bude zajímavé.' Kloor se nakloní blíž. 'Měla podivnou měnu — říkali jim Vestigely. Ne jako běžné peníze. Jsou vzácné, experimentální. Prý mají... vlastnosti. Obchodník jménem Zerren jeden od ní dostal.'",
        options: {
            tell_me_more_about_these_vestigels: "Řekněte mi víc o těch Vestigelech",
            thanks_for_the_information: "Díky za informaci"
        }
    },
    kloor_vestigels: {
        text: "'Vestigely jsou zvláštní. Jsou jako mince, ale... nějak živé. Vím o třech, které existují na trhu. Snažil jsem se jeden získat, abych studoval jeho vlastnosti. Mohl by být cenný pro můj... výzkum.'",
        options: {
            i_could_help_you_get_one: "Mohl bych vám pomoct jeden získat",
            sounds_dangerous: "To zní nebezpečně"
        }
    },
    kloor_vestigels_help: {
        text: "'Vážně? To by bylo... užitečné.' Kloorovy oči se podezíravě zúží, pak přikývne. 'Když mi seženeš jeden z Vestigelů, řeknu ti všechno, co vím o pohybech Biskupky. Platí?'",
        options: {
            deal: "Platí",
            i_need_to_think_about_it: "Musím si to rozmyslet"
        }
    },
    kloor_vestigels_quest_start: {
        text: "Výborně! Dej mi vědět, až jeden budeš mít.",
        options: { "OK": "OK" }
    },
    kloor_vestigels_quest_info: {
        text: "'Tady je, co vím: Zerren má jeden Vestigel. Obvykle bývá v tržní oblasti. Další dva drží obchodníci jménem Liss a Dovan. Najdi jednoho z nich, sežeň mi Vestigel a já ti řeknu, co potřebuješ vědět o Biskupce.'",
        options: {
            ill_find_one_for_you: "Jeden vám seženu"
        }
    },
    kloor_vestigels_progress: {
        text: "'Měl jsi štěstí při hledání Vestigelu? Ty věci se snadno neshánějí.'",
        options: {
            not_yet: "Zatím ne",
            i_have_one_right_here: "Mám jeden tady"
        }
    },
    kloor_vestigels_complete: {
        text: "Kloorovi se rozšíří oči, když mu ukážeš Vestigel. 'Neuvěřitelné! Opravdu jsi jeden našel!' Opatrně ho od tebe přebere a s fascinací ho zkoumá. 'Jak jsem slíbil, řeknu ti, co vím o Biskupce.'",
        options: {
            tell_me: "Řekněte mi"
        }
    },
    kloor_bishop_reveal: {
        text: "'Z toho, co vím, nakupovala vzácné herní předměty. Víš, naše Biskupka se stala závislou na snových hrách od Dr. Elphi Quarn. Navštěvovala Dr. Elphi celkem často. Dr. Elphi je známý herní designér, majitelka studia, kde se hry vyrábějí ze snů profesionálních imaginátorů. Je také vynálezkyní. Biskupka vypadala depresivně a něco hledala. Její kontakt s Dr. Elphi se zdá dost nepravidelný a neobvyklý. Měl bys hledat Dr. Elphi ve Škrabáku 1140.'",
        options: {
            the_dream_games: "Snové hry?"
        }
    },
    kloor_dream_games: {
        text: "'Snové hry? Ano, mohou být... návykové. Studio ARB Ambra vyrábí ty nejlepší... vlastně jediné existující. Slyšel jsem, že najít dobrého imaginátora je docela těžké. Studio je vybaveno speciálně upravenými postelemi, imaginátoři nosí na hlavách stříbrné přilby bez hledí, ke kterým je připojen složitý systém drátů, kabelů a elektrod. Přilby jsou doktorčin vynález a je na ně náležitě pyšná.'",
        options: {
            thank_you_for_the_information: "Děkuji za informace"
        }
    },
    rustDomain: {
        text: "Kloor tě zamyšleně pozoruje. 'Centrál Rezavého chóru, co? To není zrovna veřejná znalost. Ale... mohl bych ti pomoct. Za cenu, samozřejmě.'",
        options: {
            what_do_you_need: "Co potřebujete?",
            come_on_i_did_some_favors_for_the_rust_choir_alrea: "No tak, už jsem pro Rezavý chór udělal pár laskavostí. Jsme prakticky kamarádi.",
            never_mind_i_have_other_questions: "Nechte to, mám jiné otázky."
        }
    },
    factionAppeal: {
        text: "Kloor zvedne obočí a zváží tvou prosbu. 'No, asi můžu udělat výjimku pro někoho, kdo prokázal laskavosti Rezavému chóru. Jen tentokrát. Mluv s Ravlou v Řvoucím korku. Otestuje tě. Když projdeš, pustí tě dovnitř.'",
        options: {
            i_have_other_questions: "Mám jiné otázky."
        }
    },
    rustDomain_price: {
        text: "'Když mi dáš nějaké tvoje spory, můžu tě nasměrovat na někoho, kdo tě dostane dovnitř. Nebo, jestli jsou ti spory příliš drahé, pár mincí jako příspěvek taky postačí. Platí?'",
        options: {
            heres_20_spores: "Tady máte 20 spor",
            heres_50_coins: "Tady máte 50 mincí",
            you_know_what_i_have_other_questions: "Víte co, mám jiné otázky."
        }
    },
    rustDomain_spores: {
        text: "Kloor prozkoumá spory, které mu podáš, a souhlasně přikývne. 'Dobrá kvalita. Mluv s Ravlou v hospodě Řvoucí korek. Otestuje tě. Když projdeš, pustí tě dovnitř.'",
        options: {
            thanks_for_the_help: "Díky za pomoc."
        }
    },
    rustDomain_coins: {
        text: "Kloor počítá mince, které mu dáváš, a na tváři se mu rozlije spokojený úšklebek. 'Dobře se obchoduje. Teď, mluv s Ravlou v hospodě Řvoucí korek. Otestuje tě. Když projdeš, pustí tě dovnitř.'",
        options: {
            thanks_for_the_help: "Díky za pomoc."
        }
    }
};
