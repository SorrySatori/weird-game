/**
 * Czech dialog translations for ScreamingCorkScene
 * Speaker: Edgar Eskola
 */
export default {
    _speakers: {
        'Edgar Eskola': 'Edgar Eskola',
    },
    edgar_start: {
        text: "Medvědovitý tvor se neklidně zavrtí. Pohlédne na tebe se směsí ostražitosti a zvědavosti.",
        options: {
            hello_there: "Dobrý den",
            what_are_you_doing_here: "Co tady děláte?",
            tell_me_about_yourself: "Řekněte mi o sobě.",
            what_do_you_know_about_the_burning_bear_festival: "Co víte o Festivalu hořícího medvěda?",
            im_looking_for_a_vestigel_i_heard_you_might_have_o: "Hledám vestigel, slyšel jsem, že byste mohl jeden mít.",
            lets_start_with_some_inspirational_topics: "Pojďme vymyslet nějaká inspirativní témata.",
            edgar_cathedral_ask: "[Než vejdu do katedrály] Pracoval jste v tomhle městě všude možně. Existuje nějaká jiná cesta do Vaječné katedrály, o které třeba nikdo moc neví?"
        }
    },
    edgar_greeting: {
        text: "Mmm. Zdravíčko. Lidi se mnou moc často nemluví. Většina se mišutkennům vyhýbá, když to jde.",
        options: {
            why_is_that: "Proč?",
            what_are_miutkenn: "Co jsou mišutkenni?",
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_what: {
        text: "Mišutkenni jsou... no, nejsme úplně lidé. Jsme... jiní.",
        options: {
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_prejudice: {
        text: "Historie. Pověry. Strach z toho, co je jiné. Vyberte si. Zakladatelé města vyhnali mé předky z Kopřivových hor. Teď nás jen... tolerují. V lepším případě.",
        options: {
            thats_unfortunate: "To je smutné.",
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_unfortunate: {
        text: "Takhle se to dá taky říct. Ale naučil jsem se s tím žít. Většinou se to dá.",
        options: {
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_purpose: {
        text: "Čekám. Pozoruji. Vyhýbám se přípravám na ten proklatý festival. Řvoucí korek je jedno z mála míst, které nejde naplno do toho nesmyslu s pálením medvědů.",
        options: {
            you_dont_like_the_festival: "Vám se festival nelíbí?",
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_background: {
        text: "Nemám toho moc co vyprávět. Prací jsem vystřídal víc, než bych spočítal. Údržbář na Škrabáku 1140, profesionální imaginátor, úředník, model, mechanik vzdušné lodi, balič masa... V žádné z nich jsem dlouho nevydržel. Vždycky jsem za to ale nemohl.",
        options: {
            professional_imaginator: "Profesionální imaginátor?",
            why_didnt_they_work_out: "Proč to nevyšlo?",
            what_would_you_like_to_do: "Co byste rád dělal?",
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_dream_job: {
        text: "Zkoušel jsem všechno. Údržbář. Úředník. Montážník masa. Imaginátor. Ale nikdy jsem nebyl ničím skutečně svým. Myslím... že chci napsat knihu. Ale ještě nevím, o čem.",
        options: {
            i_can_help_you_write_the_book: "Mohu vám s tou knihou pomoct",
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_book: {
        text: "Vy byste... to pro mě udělal? Děkuji. Ještě nevím, o čem by měla být. Ale jsem otevřený návrhům.",
        options: {
            lets_start_with_some_inspirational_topics: "Pojďme začít s inspiračními tématy",
            i_will_come_back_when_i_have_an_idea: "Vrátím se, až budu mít nápad"
        }
    },
    edgar_book_topics: {
        text: "Je v tomhle městě něco, co vám přijde inspirativní? Máte nějaké zážitky, ze kterých by se dal vykřesat dobrý příběh? Potřebuji čerstvou perspektivu, cítím, že jsem zaseknutý.",
        options: {
            scraper_building: "Záhadná budova Škrabáku",
            lift_mother: "Inteligentní výtah Lift-Mother",
            ortolan: "Ortolan, návrhář deskových her",
            dream_queue: "Snová fronta",
            spore_infection: "Sporová infekce",
            vestigels: "Záhadné vestigely",
            kloor_venn: "Tajemný Kloor Venn",
            skyships: "Vzducholodě nad městem",
            rust_choir: "Záhadný Rezavý chór",
            burning_bear: "Festival hořícího medvěda",
            board_games_war: "Válka deskových her",
            noise_god: "Bůh hluku",
            magnekin: "Magnekin — civilizace v jednom těle",
            god_graveyard: "Hřbitov bohů pod městem",
            city_mystery: "Záhady Horní Morkezely",
            misutken_life: "Život mišutkenna ve městě"
        }
    },
    edgar_book_topic_selected: {
        text: "To je fascinující téma! Hned mě napadá pár zajímavých věcí, se kterými se dá dál pracovat.",
        options: {
            i_think_we_have_enough_topics: "Myslím, že máme dost témat",
            lets_continue: "Pokračujeme dál"
        }
    },
    edgar_book_tone: {
        text: "Dobře. Když máme nějaká témata, jaký tón by kniha měla mít? Přemýšlím o celkové atmosféře.",
        options: {
            tragic_a_tale_of_sorrow_and_loss: "Tragický — příběh smutku a ztráty",
            metaphysical_exploring_consciousness: "Metafyzický — zkoumání podstaty jsoucna",
            romantic_focusing_on_connections: "Romantický — zaměřený na vztahy",
            existential_pondering_meaning_and_mortality: "Existenciální — úvahy o smyslu života a smrtelnosti",
            political_examining_power_dynamics: "Politický — zkoumání dynamiky moci",
            comical_finding_humor_in_the_strange: "Komický — hledání humoru v podivnosti"
        }
    },
    edgar_book_tone_selected: {
        text: "To je skvělá volba! Dodá knize výrazný emocionální náboj, který by měl čtenáře oslovit hned od začátku.",
        options: {
            now_lets_choose_a_genre_for_the_book: "Teď pojďme zvolit žánr knihy"
        }
    },
    edgar_book_genre: {
        text: "Tón je nastaven, ale jaký žánr by měl příběh mít? Prozkoumával jsem některé experimentální možnosti.",
        options: {
            fungal_technothriller: "Houbový techno-thriller",
            postmodern_novel: "Postmoderní román",
            urban_fantasy: "Městská fantasy",
            funny_animals_with_depression: "Vtipná zvířata s depresí",
            detective_novel: "Detektivní román",
            dreamy_weird_fiction: "Snová weird fiction",
            mythic_war_epic: "Mýtický válečný epos",
            cosmic_horror: "Kosmický horor"
        }
    },
    edgar_book_genre_selected: {
        text: "Perfektní žánr! V tomhle divném městě by něco takového fungovat výborně.",
        options: {
            lets_decide_on_the_main_protagonist: "Pojďme vymyslet hlavní postavu"
        }
    },
    edgar_book_protagonist: {
        text: "Kdo je protagonista? Jaký typ hlavní postavy by se hodil k našemu příběhu?",
        options: {
            a_disoriented_tourist: "Dezorientovaný turista",
            a_renegade_fungal_scientist: "Houbový vědec renegát",
            a_miutkenn_seeking_identity: "Mišutkenn hledající identitu",
            an_amnesiac_with_strange_abilities: "Člověk trpící ztrátou paměti s podivnými schopnostmi",
            a_sentient_fungal_colony: "Inteligentní kolonie hub",
            a_dream_detective: "Snový detektiv",
            a_rogue_ludarch: "Ludarch odpadlík",
            a_living_collective_pretending_to_be_one_person: "Kolektiv bytostí předstírající jednu osobu"
        }
    },
    edgar_book_protagonist_selected: {
        text: "To je fascinující volba! Úplně vidím, jak takový hrdina prochází příběhem.",
        options: {
            finally_lets_choose_a_setting: "Nakonec vybereme nějaké prostředí"
        }
    },
    edgar_book_setting: {
        text: "Nakonec si musíme říct, kde by se měl tento příběh odehrávat? V jakém prostředí?",
        options: {
            the_scrapers_shifting_floors: "Proměnlivá patra Škrabáku",
            a_murderous_magical_school: "Vražedná magická škola",
            a_giant_immortal_mammal_swimming_in_the_ocean: "Obří nesmrtelný savec plovoucí v oceánu",
            the_fungal_wilds: "Houbová divočina",
            a_skyship_above_the_clouds: "Vzducholoď mezi oblaky",
            the_subterranean_markets: "Podzemní trhy",
            a_wartorn_board_game_that_became_real: "Válkou zničená desková hra, která se stala skutečností",
            the_graveyard_of_dead_gods: "Hřbitov mrtvých bohů"
        }
    },
    edgar_book_setting_selected: {
        text: "Výborné prostředí! Vytvoří dokonalou atmosféru a nabízí spoustu narativních možností.",
        options: {
            lets_see_what_book_weve_created: "Podívejme se, jakou knihu jsme vytvořili"
        }
    },
    edgar_book_completion: {
        // text se skládá za běhu a je lokalizovaný přímo ve scéně (viz BOOK_LABELS)
        options: {
            i_look_forward_to_reading_it: "Těším se, až si to přečtu",
            make_sure_to_credit_me_as_coauthor: "Nezapomeňte mě uvést jako spoluautora"
        }
    },
    edgar_book_farewell: {
        text: {
            plain: "Měl bych se pustit do práce. Mám spoustu nápadů a nechci je ztratit. Ještě jednou děkuji za pomoc. Klidně se stavte zkontrolovat, jak mi to jde.",
            vestigel: "Měl bych se pustit do práce. Mám spoustu nápadů a nechci je ztratit. Ach, a jak jsem slíbil — tady je ten vestigel. Vám se bude hodit víc než mně. Ještě jednou díky za pomoc. Klidně se stavte zkontrolovat, jak mi to jde."
        },
        options: {
            good_luck_edgar: "Hodně štěstí, Edgare"
        }
    },
    edgar_imaginator: {
        text: "Vymýšlel jsem lokace a postavy pro hry Dr. Elphi Quarn. Ukázalo se, že moje představivost byla příliš... divoká. Příliš chaotická, říkali. Moje sny byly prý 'nepoužitelné.' Jejich smůla.",
        options: {
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_jobs: {
        text: "Většinou byl problém se špatným načasováním. Škrabák převzal Rezavý chór a přestal být oficiální součástí města — údržbář už nebyl potřeba. Ty ostatní práce... no, být mišutkennem tu moc s hledáním práce nepomáhá.",
        options: {
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_festival: {
        text: "Festival hořícího medvěda? Krutá připomínka starodávného 'vítězství' nad mým druhem. Vycpou medvědí kůže věcmi, kterých se chtějí zbavit, a pak je o půlnoci spálí. Někteří je plní hmyzem a domácími škůdci, symboly zlozvyků, neřestmi... ale kolují drby o lidech, kteří do nich zašívají své soupeře a rivaly.",
        options: {
            that_sounds_disturbing: "To se mi moc nelíbí.",
            its_just_tradition_isnt_it: "Je to přece jen tradice, ne?",
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_disturbing: {
        text: "To teda. Představte si, že jste obklopen hořícími figurínami, které vypadají jako vaši předkové. Po městě se rozsvítí ohně různých barev a vůní, zatímco já se schovávám a čekám, až to skončí.",
        options: {
            im_sorry_to_hear_that: "To je mi líto.",
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_tradition: {
        text: "Tradice? Tradice mohou být kruté. To, že se něco dělá po generace, neznamená, že je to správné. Ale málokdo v tomhle městě by se mnou souhlasil.",
        options: {
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_sympathy: {
        text: "Váš soucit je... nečekaný. Ale díky za něj. Možná není každý v tomhle městě tak bezohledný, jak jsem si myslel.",
        options: {
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_vestigel: {
        text: "Vestigel? Ano... jeden mi opravdu patří. Je to zvláštní předmět, malá, ale zjevně cenná mince. Byla schovaná uvnitř plyšáka. Vidíte, koupil jsem ten vestigel nevědomky od pouliční prodavačky. Jinak by někdo toho plyšáka použil na ten prokletý festival. Prodavačka o vestigelu taky nevěděla, ale překvapivě odmítla, abych jí ho vrátil. Říkala něco o profesionální cti, hmm...",
        options: {
            i_need_it_for_an_important_purpose: "Potřebuji ho na něco důležitého.",
            may_i_have_it: "Mohl byste mi ho dát?",
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_vestigel_need: {
        text: "Důležitý účel, říkáte? No, já ho vlastně *nepotřebuji*, ale docela se mi líbí. Možná byste pro mě mohl něco udělat na oplátku?",
        options: {
            what_do_you_need: "Co potřebujete?",
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_vestigel_request: {
        text: "Jen tak? Víte, že to má nějakou cenu. Nedal bych to pryč bez dobrého důvodu.",
        options: {
            what_would_convince_you_to_part_with_it: "Co by vás přesvědčilo se ho vzdát?",
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_vestigel_convince: {
        text: "Hmm...",
        options: {
            i_already_helped_you_write_your_book: "Už jsem vám pomohl napsat knihu.",
            i_could_help_with_your_book_as_we_discussed_earlie: "Mohl bych pomoct s vaší knihou, jak jsme se bavili dříve.",
            maybe_i_could_help_you_with_something: "Možná bych vám mohl s něčím pomoct.",
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_vestigel_book_help: {
        text: "Ano, s knihou jste mi pomoc nabídl. Férová výměna — vaše pomoc za vestigel. Sbírám nápady, ale moc daleko jsem se nedostal.",
        options: {
            ill_make_sure_your_book_becomes_a_reality: "Postarám se, aby vaše kniha vznikla."
        }
    },
    edgar_vestigel_offer: {
        text: "Hmm... možná byste mi mohl s něčím pomoct. Víte něco o literatuře? Rád bych se stal... spisovatelem. Ale nevím, kde začít. Snažím se najít hlas, příběh, který stojí za vyprávění. Pokud byste mi opravdu mohl pomoct...",
        options: {
            ill_do_my_best: "Udělám, co bude v mých silách."
        }
    },
    edgar_vestigel_give_completed: {
        text: "Aha, zajímá vás vestigel? Po tom všem, jak jste mi pomohl s mojí knihou, vám ho rád dám. Asi pro něj najdete lepší využití než já. Tady, vezměte si ho. Ještě jednou díky.",
        options: {
            thank_you_edgar: "Děkuji, Edgare."
        }
    },
    edgar_vestigel_thanks: {
        text: "Nezapomeňte na svůj slib. Těším se na to, co spolu vytvoříme. Kniha, která skutečně zachytí podstatu... no, toho, co musíme ještě vymyslet.",
        options: {
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_cathedral_way: {
        text: "Medvědovitý tvor úplně znehybní — tak jako zvířata, když zaslechnou něco potenciálně nebezpečného. \"Vaječná katedrála... Ano. Přemýšlel jsem o tom místě víc než většina lidí.\" Ztiší hlas. \"Zametal jsem její chodby. Mazal její výtahové klece. Vynášel její popel. Nikdo nesleduje toho, kdo vynáší popel. A naučil jsem se něco, co ti kněží nikdy: všichni používali hlavní vchod. Velké dveře. Strážce. Zato já jsem vždycky hledal místa, která nejsou na první pohled vidět. Některé noci ty chrámové zdi dýchají.\"",
        options: {
            the_walls_breathe: "Zdi dýchají?",
            then_show_me_the_way_in: "Ukažte mi tu cestu dovnitř.",
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_cathedral_breathe: {
        text: "\"Dýchají. Nádech a výdech, pomalu, jako by tam něco spalo. Všichni si myslí, že sklepy katedrály jsou jen mrtvý a prázdný prostor, kde nic není. Myslím ale, že mrtvé nejsou. Něco tam dole je. Upřímně, radši jsem tam přestal pracovat, protože jsem dostal strach. Netuším, jestli se ty chodby od té doby nějak proměnily, na to už se budete muset podívat sám.\"",
        options: {
            so_there_is_a_way_in: "Takže cesta dovnitř existuje.",
            back_to_other_topics: "Zeptej se na něco jiného"
        }
    },
    edgar_cathedral_path: {
        text: "\"Existuje. Na severní straně, kde se zeď katedrály stýká s vrstvou mrtvých bohů, je místo, kde se neustále pohybuje půda a vzniká úzký průlez. Škvíra na šířku medvědích ramen, pokud vám nevadí zápach z podzemí. Jděte radši hned. Ať už se tam uvnitř probouzí cokoli, škvíru dlouho otevřenou nenechá.\" Zaváhá, jednu velkou tlapu položenou na stole. \"Buďte opatrný. Vždycky jsem o sobě uvažoval jako o někom, kdo si všímá toho, čeho si nikdo jiný nevšimne. Nerad bych byl posledním, kdo vás viděl živého.\"",
        options: {
            thank_you_edgar_cathedral: "Děkuji, Edgare.",
            i_should_go: "Měl bych jít."
        }
    }
};
