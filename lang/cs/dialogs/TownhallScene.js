/**
 * Czech dialog translations for TownhallScene
 * Speaker: Phor Calesta
 */
export default {
    _speakers: {
        'Phor Calesta': 'Phor Calesta',
    },
    phorGreeting: {
        text: "Ach, další poutník do archivů zapomenutého božství. Jsem Phor Calesta, archeolog ztracených teologií. Místní mi říkají 'Kopáč bohohrobů', i když já preferuji termín divinograf.",
        options: {
            townhall_granted_permit: "Dobrá zpráva — Radnice vám schválila povolení k vykopávkám.",
            what_is_divinography: "Co je divinografie?",
            where_are_you_from: "Odkud jste?",
            what_are_you_doing_here: "Co tu děláte?",
            im_looking_for_a_living_redmass: "Hledám živou rudohmotu"
        }
    },
    phorAccessGranted: {
        text: "Phor Calesta málem upustí notes. \"Oni — oni to *schválili*? Po tom všem čekání?\" Vzchopí se, oči jí září. \"Tak není času nazbyt. Cesta dolů se otevírá z Náměstí — staré schodiště do Vaječné katedrály. Jděte tam a sejděte dolů; já vás počkám u brány. Konečně — někdo, kdo se mnou bude číst mrtvoly.\"",
        options: {
            meet_at_lower_doors: "Sejdeme se u spodních dveří."
        }
    },
    phorDivinography: {
        text: "Divinografie je studium mrtvých bohů jako geologických jevů. Když božstvo zemře, jeho esence jednoduše nezmizí — zkamení. Zkostnatělé modlitby se stávají vrstvami. Svatozáře krystalizují do nerostných ložisek. Já tyto božské pozůstatky vykopávám a katalogizuji jejich vlastnosti.",
        options: {
            that_sounds_like_grave_robbery: "To zní jako vykrádání hrobů",
            fascinating_what_have_you_found: "Fascinující. Co jste našel?",
            ask_something_else: "Zeptejte se na něco jiného"
        }
    },
    phorGraveRobbery: {
        text: "(Vlhce se zasměje) Možná. Ale řekněte mi — je to krádež, když zesnulý nemá žádné dědice? Žádné uctívače? Když bůh zemře a jeho věrní se rozprchnou, jeho pozůstatky se stávají... nezjištěným majetkem. Já jednoduše zajišťuji, aby byly studovány místo zapomenuty.",
        options: {
            i_suppose_thats_one_way_to_look_at_it: "Předpokládám, že to je jeden úhel pohledu",
            what_have_you_found: "Co jste našel?"
        }
    },
    phorFindings: {
        text: "Pozoruhodné věci. Bohokov, který hučí reziduální vírou. Růžence, které stále šeptají modlitby v mrtvých jazycích. Jednou jsem našel fragment svatozáře, který promítal poslední myšlenky svého božstva — smyčku kosmického zoufalství trvající přesně 47 sekund.",
        options: {
            thats_disturbing: "To je znepokojující",
            where_do_you_find_these_things: "Kde tyto věci nacházíte?",
            ask_something_else: "Zeptejte se na něco jiného"
        }
    },
    phorDisturbing: {
        text: "Znepokojující? Možná. Ale také poučné. Z božské smrti se dozvíme víc než z božského života. Bohové lžou svým následovníkům. Jejich mrtvoly říkají jen pravdu.",
        options: {
            ask_something_else: "Zeptejte se na něco jiného"
        }
    },
    phorLocations: {
        text: "Kdekoliv se protíná tlak a paměť. Zatopené chrámy. Zřícené katedrály. Hlubší vrstvy tohoto samotného města. Murkvale, můj domov, je obzvláště bohatý — vodní tlak tam krásně fosilizuje božskou esenci.",
        options: {
            tell_me_about_murkvale: "Řekněte mi o Murkvale",
            ask_something_else: "Zeptejte se na něco jiného"
        }
    },
    phorMurkvale: {
        text: "Jsem z Murkvale. Je to zatopené město, kde se můj lid, Craybara, vyvinul. Jsme obojživelní — přizpůsobení drtivým hloubkám i sociálním tlakům. Ruiny města rostou jako korály a samotná paměť fosilizuje v sedimentu. Je to perfektní laboratoř pro studium božské archeologie.",
        options: {
            what_brought_you_here: "Co vás sem přivedlo?",
            ask_something_else: "Zeptejte se na něco jiného"
        }
    },
    phorPurpose: {
        text: "Toto město stojí na vrstvách mrtvých věr. Vaječná katedrála nahoře je pouze nejnovější iterací. Pod městem leží kosti mnoha bohů, kteří sem přišli zemřít. Jsem tu, abych vykopal spodní vrstvy a zdokumentoval, jací bohové zemřeli, aby uvolnili místo současným. Ale k tomu musím nejprve přežít byrokracii. Bohužel, radnice mi nedovolí kopat na úrovni Bohohřbitova bez řádných povolení. Ale ta zatracená radnice je zavřená, nebo co.",
        options: {
            thats_quite_ambitious: "To je docela ambiciózní",
            can_i_help_you_to_get_the_permission: "Mohu vám pomoct získat povolení?",
            the_townhall_is_closed_i_need_to_get_in_there_too: "Radnice je zavřená? Já tam taky potřebuji.",
            ask_something_else: "Zeptejte se na něco jiného"
        }
    },
    phorPurposeHelp: {
        text: "Hmm. Možná byste mohl. Pokud se vám podaří prokousat byrokracií radnice a zajistit povolení k vykopávkám na Bohohřbitově, budu vám navěky vděčný.",
        options: {
            ill_see_what_i_can_do: "Uvidím, co se dá dělat"
        }
    },
    phorAmbitious: {
        text: "Ambice jsou vše, co přežije tlak. V Murkvale jsme se to naučili brzy. Slabí jsou rozdrceni. Ambiciózní se přizpůsobí. Hodlám publikovat definitivní text o božské stratigrafii — za předpokladu, že přežiji vykopávky.",
        options: {
            ask_something_else: "Zeptejte se na něco jiného"
        }
    },
    phorTownhallClosed: {
        text: "Zavřená, ano. Nikdo přesně neví proč. Úředníci jednoho dne prostě přestali chodit. Někteří říkají, že jde o byrokratickou restrukturalizaci. Jiní říkají, že budova sama odmítá návštěvníky — víte, jak to v tomhle městě chodí, budovy si vytvářejí názory.\n\nČekám tu už několik dní. Kdybyste našel cestu dovnitř, byl bych vám neskonale vděčný.",
        options: {
            ill_figure_something_out: "Něco vymyslím."
        }
    },
    phorAskSomethingElse: {
        text: "Ano? Co dalšího trápí vaši zvědavost?",
        options: {
            what_is_divinography: "Co je divinografie?",
            tell_me_about_murkvale: "Řekněte mi o Murkvale",
            what_are_you_doing_here: "Co tu děláte?",
            im_looking_for_a_redmass: "Hledám rudohmotu"
        }
    },
    phorRustCluster: {
        text: "Ach! Živá rudohmota. Fascinující organismy — zčásti minerál, zčásti rezonance víry, zčásti korozní organismus. Rostou na místech, kde kov a rozklad dosáhnou rovnováhy. Ve svých vykopávkách jsem zdokumentoval několik kolonií.",
        options: {
            where_can_i_find_one: "Kde mohu nějakou najít?",
            what_are_they_exactly: "Co přesně jsou?"
        }
    },
    phorRustClusterLocation: {
        text: "Nejpřístupnější kolonie, kterou jsem našel, je v nižších úrovních Kůlny 521 — konkrétně v údržbářských halách. Byrokrati se té oblasti vyhýbají kvůli 'konstrukčním problémům', což ji činí perfektní pro růst rudohmoty. Hledejte zkorodované potrubí s oranžovo-rudými krystalickými výrůstky.",
        options: {
            how_do_i_extract_one_safely: "Jak ji bezpečně získám?",
            thank_you_for_the_information: "Děkuji za informaci"
        }
    },
    phorRustClusterNature: {
        text: "Jsou to symbiotické organismy — houbové mycelium, které se živí oxidujícím kovem a zároveň uchovává strukturální paměť toho, čím kov kdysi byl. V jistém smyslu jsou to živé fosilie rozkladu. Docela poetické, vlastně.",
        options: {
            where_can_i_find_one: "Kde mohu nějakou najít?",
            ask_something_else: "Zeptejte se na něco jiného"
        }
    },
    phorRustClusterExtraction: {
        text: "Opatrně. Jsou křehké. Použijte čepel k oddělení kolonie od podkladu, ale nechte trochu hostitelského kovu připojenou — potřebují ho k přežití. A ať děláte cokoliv, nevystavujte je čisté vodě. Narušuje to jejich oxidační cyklus.",
        options: {
            thank_you_for_the_advice: "Děkuji za radu"
        }
    },
    
    phorRustClusterThanks: {
        text: "Samozřejmě. Pokud budete úspěšný, řekněte mi, k čemu ji používáte. Vždy mě zajímá, jak ostatní využívají tyto božské pozůstatky.",
        options: {
            ask_something_else: "Zeptejte se na něco jiného"
        }
    }
};
