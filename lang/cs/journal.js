/**
 * Czech journal entry translations — keyed by journal entry ID
 */
export default {
    journal: {
        heard_stomach_clock: {
            title: 'Žaludeční hodiny',
            description: 'Někdo mi vyprávěl o Žaludečních hodinách na radnici: biomechanická komora ve tvaru trávicích hodin, kde čas běží v kruzích a žluč je posvátná.',
        },
        obazoba_cult: {
            title: 'Kult Obazoba',
            description: 'Kult Obazoba je kult houbových uctívačů, kteří uctívají Obazobu, mýtickou houbovou entitu.',
        },
        upper_morkezela: {
            title: 'Horní Morkezela',
            description: 'Říkají tomu město, ale Horní Morkezela nikdy nebyla plně živá. Přesto dýchá — něco dýchá. Ulice jsou vyrostlé, ne postavené.',
        },
        fungal_apprentice: {
            title: 'Houbový Učeň',
            description: 'Jako nejnovější učeň Houbového Mistra Thaala se musím hodně naučit o světě spór a mycelia.',
        },
        fungal_master: {
            title: 'Houbový Mistr Thaal',
            description: 'Výstřední Houbový Mistr Thaal je známý svými rozsáhlými znalostmi spór a hub.',
        },
        growthDecay: {
            title: 'Růst a Rozklad',
            description: 'Moje činy mohou změnit město a jeho budoucnost. Růst živí to, co žije; Rozklad strhává to, co přetrvává.',
        },
        abandoned_bus_interior: {
            title: 'Interiér Opuštěného Autobusu',
            description: 'Vnitřek autobusu je podivná směs rozpadu a nového růstu.',
        },
        bishop_bruising: {
            title: 'Neurální Trauma',
            description: 'Tělo Biskupky vykazuje známky neurálního traumatu — modřiny na spáncích, kde se připojují zařízení snového rozhraní.',
        },
        bishop_cartridge: {
            title: 'Kardinálská Hostina',
            description: 'Biskupka používala snovou kazetu nazvanou „Kardinálská Hostina", když zemřela.',
        },
        bishop_helmet: {
            title: 'Helma Snového Rozhraní',
            description: 'Biskupka měla na sobě přenosnou helmu snového rozhraní, když zemřela.',
        },
        bishop_journal: {
            title: 'Deník Biskupky',
            description: 'Deník Biskupky obsahuje znepokojivou poznámku: „Město mě už neslyší. Možná to dokážou sny."',
        },
        bishop_memo: {
            title: 'Poznámka z Radnice',
            description: 'Poznámka od Biskupky popisuje znepokojivé setkání s něčím, co zní jako dvojník.',
        },
        bishop_dissection: {
            title: 'Neobvyklý Houbový Výrůstek',
            description: 'Při prohlídce těla Biskupky jsem objevil neobvyklý houbový výrůstek.',
        },
        neme_symbiont_discovered: {
            title: 'Neme z Korunobažiny',
            description: 'Objevil jsem symbionta žijícího uvnitř mrtvého těla Biskupky.',
        },
        neme_symbiont_accepted: {
            title: 'Spojen s Neme',
            description: 'Přijal jsem symbionta Neme z Korunobažiny. Jeho úponky se uhnízdily pod mou kůží.',
        },
        neme_symbiont_declined: {
            title: 'Odmítnutí Neme',
            description: 'Rozhodl jsem se nepřijmout symbionta, který se vynořil z těla Biskupky.',
        },
        bishop_berries: {
            title: 'Kořeněné Mručenky',
            description: 'Biskupka krátce před smrtí jedla Mručenky.',
        },
        symbiont_thorne_still_offered: {
            title: 'Setkání s Trnem-Klidem',
            description: 'V podivné mrtvole na křižovatce jsem našel symbiotickou entitu, která si říká Trn-Klid.',
        },
        crossroad_corpse_spores_planted: {
            title: 'Zasazeny Spóry v Mrtvole na Křižovatce',
            description: 'Zasadil jsem spóry do podivné mrtvoly na křižovatce.',
        },
        symbiont_thorne_still_declined: {
            title: 'Odmítnutí Trnu-Klidu',
            description: 'Odmítl jsem nabídku od symbiotické entity Trn-Klid.',
        },
        symbiont_thorne_still_accepted: {
            title: 'Přijetí Trnu-Klidu',
            description: 'Přijal jsem symbiotickou entitu Trn-Klid. Splynula se mnou.',
        },
        skyship_sighting: {
            title: 'Podivné Plavidlo na Smaragdové Obloze',
            description: 'Nad Horní Morkezelou se vznáší obrovská vzdušná loď.',
        },
        delta_fungal_bridge: {
            title: 'Houbový Most v Deltě Ozvěnového Odpadu',
            description: 'Zasadil jsem spóry do sedimentu mezi ostrovy delty.',
        },
        redmass_encountered: {
            title: 'Živá Rudá Hmota',
            description: 'Na ostrově v Deltě Ozvěnového Odpadu jsem našel živou rudou hmotu — krystalický kovový organismus, který umí mluvit.',
        },
        redmass_spared: {
            title: 'Ušetřena Rudá Hmota',
            description: 'Rozhodl jsem se nevzít živou rudou hmotu silou.',
        },
        redmass_collected_forced: {
            title: 'Odtržena Rudá Hmota',
            description: 'Odtrhl jsem živou rudou hmotu z ostrova navzdory jejím prosbám.',
        },
        redmass_collected_voluntary: {
            title: 'Dobrovolně Obdržena Rudá Hmota',
            description: 'Živá rudá hmota na ostrově mi dobrovolně dala kousek sebe.',
        },
        symbiont_ulvarex_accepted: {
            title: 'Přijat Ulvarex Vypůjčený Obzor',
            description: 'V přístavu jsem narazil na symbionta uvězněného v odrazu vody.',
        },
        symbiont_ulvarex_declined: {
            title: 'Odmítnut Ulvarex Vypůjčený Obzor',
            description: 'Narazil jsem na podivného symbionta v přístavní vodě.',
        },
        scraper_building: {
            title: 'Škrabák',
            description: 'Impozantní stavba známá jako „Škrabák" se tyčí nad městem.',
        },
        lift_mother_meeting: {
            title: 'Matka Výtahu',
            description: 'Uvnitř budovy Škrabáku jsem narazil na nejneobvyklejší vědomí — výtah.',
        },
        accessed_elphi_floor: {
            title: 'Studio Dr. Elphi — Patro 177-Ticho',
            description: 'Získal jsem přístup do studia Dr. Elphi Quarnové v patře 177-Ticho.',
        },
        rust_choir_info: {
            title: 'Sídlo Rezavého chóru',
            description: 'Rezavý chór sídlí v horních patrech budovy Škrabáku.',
        },
        scraper_backyard: {
            title: 'Dvorek Škrabáku',
            description: 'Za budovou Škrabáku leží zarostlý dvůr.',
        },
        elphi_told_bishop_dead: {
            title: 'Dr. Elphi se Dozvídá o Smrti Biskupky',
            description: 'Řekl jsem Dr. Elphi, že Biskupka je mrtvá.',
        },
        elphi_dream_kill_theory: {
            title: 'Smrt Rekurzivním Zážitkem',
            description: 'Dr. Elphi vysvětlila, že snové helmy mají neurální pojistky.',
        },
        elphi_bruising_analysis: {
            title: 'Analýza Dr. Elphi: Neurální Modřiny',
            description: 'Dr. Elphi potvrdila, že modřiny odpovídají neurálnímu zpětnému rázu.',
        },
        elphi_cartridge_analysis: {
            title: 'Analýza Dr. Elphi: Kardinálská Hostina',
            description: 'Dr. Elphi prozkoumala kazetu Kardinálské Hostiny.',
        },
        elphi_helmet_analysis: {
            title: 'Analýza Dr. Elphi: Snová Helma',
            description: 'Dr. Elphi potvrdila, že poškozený port neurálního rozhraní vykazuje extrémní signálovou zátěž.',
        },
        elphi_memo_analysis: {
            title: 'Analýza Dr. Elphi: Dvojník',
            description: 'Dr. Elphi byla skutečně znepokojená poznámkami Biskupky o dvojníkovi.',
        },
        elphi_townhall_log: {
            title: 'Záznamy Radnice o Dvojníkovi',
            description: 'Poznámky o Biskupčině setkání s dvojníkem byly napsané na oficiálních radničních papírech.',
        },
        elphi_journal_analysis: {
            title: 'Analýza Dr. Elphi: Deník Biskupky',
            description: 'Dr. Elphi interpretovala zápis v deníku Biskupky.',
        },
        elphi_dissection_analysis: {
            title: 'Analýza Dr. Elphi: Houbový Výrůstek',
            description: 'Dr. Elphi identifikovala houbový výrůstek jako symbionta.',
        },
        elphi_berries_analysis: {
            title: 'Analýza Dr. Elphi: Kořeněné Mručenky',
            description: 'Dr. Elphi potvrdila, že Mručenky jsou kořeněná odrůda.',
        },
        elphi_lumen_lead: {
            title: 'Stopa Vyšetřování: Lumen Direktoriát',
            description: 'Dr. Elphi mě nasměrovala k Lumen Direktoriátu.',
        },
        elphi_ready_for_day2: {
            title: 'Den 1 Dokončen: Vyšetřování Začíná',
            description: 'Sdílel jsem všechny své nálezy s Dr. Elphi Quarnovou.',
        },
        gnur_meeting: {
            title: 'Gnur z Rezavého chóru',
            description: 'V útrobách Skladu 521 jsem narazil na Gnura.',
        },
        rust_choir_faction: {
            title: 'Rezavý chór — Stroje a Paměť',
            description: 'Rezavý chór se zdá být frakcí se zájmem o starou technologii.',
        },
        ortolan_meeting: {
            title: 'Ortolan — Návrhář Deskových Her',
            description: 'Setkal jsem se s Ortolanem, vývojářem deskových her.',
        },
        board_games_war: {
            title: 'Válka Deskových Her',
            description: 'Po Starých Válkách města řešila spory prostřednictvím her.',
        },
        phor_calesta: {
            title: 'Phor Calesta — Divinograf',
            description: 'Setkal jsem se s dalším podivným člověkem. Archeologem specializujícím se na studium mrtvých bohů.',
        },
        kloor_venn_meeting: {
            title: 'Kloor Venn — Farmaceutický Podnikatel',
            description: 'Na Voxmarketu jsem narazil na Kloora Venna.',
        },
        kloor_vestigel_exchange: {
            title: 'Výměna Vestigelu s Kloorem',
            description: 'Vyměnil jsem vestigel získaný od Edgara Eskoly s Kloorem Vennem.',
        },
        bishop_elphi_contact: {
            title: 'Kontakt Biskupky s Dr. Elphi',
            description: 'Podle Kloora Venna Biskupka poměrně často navštěvovala Dr. Elphi Quarnovou.',
        },
        pith_reclaimers_faction: {
            title: 'Dřeňoví Reklamátoři — Strážci Rovnováhy',
            description: 'Dřeňoví Reklamátoři se zdají být frakcí oddanou udržování rovnováhy.',
        },
        forgotten_elevator_button_available: {
            title: 'Záhadný Knoflík u Zerren',
            description: 'Zerren přidala do svého sortimentu podivný předmět.',
        },
        rust_feast_completed_full: {
            title: 'Rezavá Hostina Připravena',
            description: 'Ravla připravila Rezavou Hostinu s živou rudou hmotou, kterou jsem přinesl.',
        },
        rust_feast_completed_shard: {
            title: 'Rezavá Hostina Připravena (Skromná)',
            description: 'Ravla připravila Rezavou Hostinu se střepem rudé hmoty, který jsem přinesl.',
        },
        rust_feast_completed_illusion: {
            title: 'Rezavá Hostina Připravena (Iluzorní Rudá Hmota)',
            description: 'Použil jsem Ulvarexovu Přeludovou Tkaninu k vytvoření iluze rudé hmoty.',
        },
        rust_feast_completed_poisoned: {
            title: 'Rezavá Hostina Připravena (Otrávená)',
            description: 'Než Ravla zamíchala Rezavou hostinu, rozetřel jsem do oleje žíravý kultivar Lumenova direktoriátu. Ničeho si nevšimla — ale až se stroje nakrmí, sežere je zevnitř.',
        },
        ravla_feast_refused_spared: {
            title: 'Ravla Odmítla — Rudá Hmota Ušetřena',
            description: 'Ravla zuřila, že jsem ušetřil rudou hmotu.',
        },
        heliodor_lore: {
            title: 'Heliodor Donjon Vaalstran',
            description: 'Heliodor je jedinečný jedinec složený z více symbiotických tvorů.',
        },
        rust_domain_arrival: {
            title: 'Rezavá Doména',
            description: 'Dostal jsem se do horních pater Škrabáku.',
        },
        rust_choir_joined: {
            title: 'Přijat do Rezavého chóru',
            description: 'Doručil jsem Rezavou Hostinu Brukkovi.',
        },
        rust_choir_rejected: {
            title: 'Odmítnut Rezavým chórem',
            description: 'Doručil jsem Rezavou Hostinu, ale střep byl příliš skrovný.',
        },
        rust_choir_probationary: {
            title: 'Rezavý chór: Ve zkušební době',
            description: 'Mé místo v Rezavém chóru je jen prozatímní. Brukk mě přijal na sílu mé pověsti, ne za pořádnou hostinu — jsem ve zkušební době, dokud nedokážu svou oddanost železu.',
        },
        rust_choir_full_member: {
            title: 'Rezavý chór: Plný člen',
            description: 'Dokázal jsem svou oddanost železu a Brukk zrušil mou zkušební dobu — teď jsem plnohodnotný člen Rezavého chóru.',
        },
        rust_choir_machines_destroyed: {
            title: 'Stroje Umlkly',
            description: 'Iluzorní rudá hmota, kterou jsem použil v Rezavé Hostině, zničila stroje Rezavého chóru.',
        },
        captain_liris_meeting: {
            title: 'Kapitán Liris z Verdigrace',
            description: 'Na palubě vzdušné lodi jsem se setkal s Kapitánem Lirisem.',
        },
        lumen_directorate_faction: {
            title: 'LUMEN DIREKTORIÁT',
            description: 'Lumen Direktoriát je mocná frakce.',
        },
        floor_counter_tool: {
            title: 'Kalibrační Nástroj Výtahu',
            description: 'Kapitán Liris mi poskytl speciální kalibrační nástroj.',
        },
        met_gardener_verrik: {
            title: 'Setkání se Zahradníkem Verrikem',
            description: 'Potkal jsem zahradníka jménem Verrik před Lumen Direktoriátem.',
        },
        verrik_gave_cultivar: {
            title: 'Kultivar direktoriátu',
            description: 'Verrik mi dal zakázaný kultivar direktoriátu — řízenou hnilobu vyšlechtěnou žrát kov. Rozetřený do oleje a předložený strojům Rezavého chóru je zkoroduje zevnitř.',
        },
        gardener_bishop_visits: {
            title: 'Návštěvy Biskupky u Lumen Direktoriátu',
            description: 'Zahradník Verrik prozradil, že Biskupka pravidelně navštěvovala.',
        },
        grew_mushroom_verrik: {
            title: 'Vyrostla Houba pro Verrika',
            description: 'Obětoval jsem spóry u Lumen Direktoriátu a vyrostl houbu.',
        },
        lumen_directorate_joined: {
            title: 'Člen Lumenova direktoriátu',
            description: 'Tím, že jsem ochromil strojovou svatyni Rezavého chóru, jsem dokázal svou oddanost růstu nad stagnací. Korektor úhlů mě uvedl jako plného člena Lumenova direktoriátu: Nic skrytého, nic ztraceného.',
        },
        met_angle_corrector: {
            title: 'Setkání s Korektorem Úhlu',
            description: 'Setkal jsem se se záhadným Korektorem Úhlu ve třetím patře.',
        },
        ac_growth_test_passed: {
            title: 'Úspěšný Test Korektora Úhlu',
            description: 'Korektor Úhlu otestoval mé porozumění růstu.',
        },
        ac_sulkberry_records: {
            title: 'Záznamy o Dodávkách Mručenek',
            description: 'Korektor Úhlu potvrdil, že Biskupka dostávala prémiové kořeněné Mručenky.',
        },
        sulkberries_cleared_verrik: {
            title: 'Verrik: Mručenky Byly Čisté',
            description: 'Zahradník Verrik potvrdil, že v pěstování mručenek nebyla žádná kontaminace.',
        },
        sulkberries_neme_verrik: {
            title: 'Neme Potvrzuje: Verrik Mluví Pravdu',
            description: 'Použil jsem Nemovu Fotosenzitivitu k ověření Verrikova tvrzení. Žádný podvod nezjištěn.',
        },
        sulkberries_cleared_kloor: {
            title: 'Kloor Venn: Mručenky Jsou Čisté',
            description: 'Kloorova chemická analýza potvrdila, že mručenky neobsahují příměsi ani toxiny.',
        },
        sulkberries_neme_kloor: {
            title: 'Neme Potvrzuje: Kloor Mluví Pravdu',
            description: 'Použil jsem Nemovu Fotosenzitivitu k ověření Kloorovy analýzy. Žádný podvod nezjištěn.',
        },
        sulkberries_cleared_heliodor: {
            title: 'Heliodor: Mručenky Jsou Čisté',
            description: 'Heliodorovy kompozitní organismy provedly multismyslovou analýzu. Mručenky jsou čisté.',
        },
        sulkberries_neme_heliodor: {
            title: 'Neme Potvrzuje: Heliodor Mluví Pravdu',
            description: 'Použil jsem Nemovu Fotosenzitivitu na Heliodorovu kolonii. Vícenásobné poctivé signály potvrzeny.',
        },
        ac_cathedral_tension: {
            title: 'Zapečetění Katedrály — Pohled Korektora Úhlu',
            description: 'Korektor Úhlu byl zjevně zasažen rozhodnutím Biskupky zapečetit Vaječnou Katedrálu.',
        },
        met_seldo: {
            title: 'Setkání se Seldem Třikrát-Korigovaným',
            description: 'Setkal jsem se se Seldem Třikrát-Korigovaným ve druhém patře Lumen Direktoriátu. Řeší byrokratickou stykou linku s městem.',
        },
        seldo_auction_errand: {
            title: 'Seldův Úkol: Aukce na Voxmarketu',
            description: 'Seldo chce, abych na aukci na Voxmarketu koupil Chronobřečkové Ropušátko jeho jménem — vzácnou ropuchu, která po olíznutí umožní vidět tři minuty do budoucnosti. Na oplátku mi dá náhradní klíč od Radnice.',
        },
        edgar_eskola_meeting: {
            title: 'Edgar Eskola — Mišutkenn Křičícího Koreku',
            description: 'Setkal jsem se s Edgarem Eskolou, patronem mišutkenn v Křičícím Koreku.',
        },
        edgar_aspiration: {
            title: 'Literární Aspirace Edgara',
            description: 'Edgar Eskola vyjádřil svou touhu napsat knihu.',
        },
        edgar_book_quest_start: {
            title: 'Slib Spisovateli',
            description: 'Nabídl jsem se, že pomůžu Edgaru Eskolovi napsat jeho knihu.',
        },
        edgar_vestigel_received: {
            title: 'Spisovatelův Žeton',
            description: 'Dnes jsem získal jeden ze tří vestigelů od Edgara Eskoly.',
        },
        edgar_vestigel_negotiation: {
            title: 'Dohoda s Edgarem',
            description: 'Edgar Eskola souhlasil s výměnou svého vestigelu za pomoc.',
        },
        edgar_vestigel_agreement: {
            title: 'Vestigelový Obchod',
            description: 'Edgar souhlasil, že mi dá svůj vestigel, jakmile mu pomůžu napsat jeho knihu.',
        },
        feral_toast_performance: {
            title: 'Koncert Feral Toast',
            description: 'Zažil jsem mysl ohýbající zvuky Feral Toast.',
        },
        noise_god_insight: {
            title: 'Vhled do Boha Hluku',
            description: 'Během zkoušky Feral Toast jsem zažil hluboký vhled do Boha Hluku.',
        },
        feral_toast_first_visit: {
            title: 'Zkouška Feral Toast',
            description: 'Navštívil jsem Klub Křičící Korek a zažil zkoušku Feral Toast.',
        },
        magnekin_hopsalot_conversion: {
            title: 'Konverze Maltimus Hopsalot',
            description: 'Konvertoval jsem Magnekina na víru Maltimuse Hopsalota.',
        },
        magnekin_hopsalot_church: {
            title: 'Kostel Maltimuse Hopsalota',
            description: 'Lagerlandie je mýtická země, kde pivo teče jako řeky.',
        },
        magnekin_reveal: {
            title: 'Magnekin',
            description: 'Zjistil jsem, že tvor jménem Magnekin není jediná bytost.',
        },
        magnekin_destroyed: {
            title: 'Zničení Magnekina',
            description: 'Zničil jsem Magnekina.',
        },
        magnekin_destroyed_mercy: {
            title: 'Zničení Magnekina',
            description: 'Zničil jsem Magnekina... ale když Rudá Hmota prosila o milost, nechal jsem ji jít.',
        },
        met_hesh_vell: {
            title: 'Dvojčata-likitátoři: Hesh a Vell',
            description: 'Setkal jsem se s dvojčaty-likitátory v Aukční hale Voxmarketu. Jeden mluví, druhý bezhlasně artikuluje tatáž slova o půl sekundy později. Jejich synchronizace je znepokojivá a přesná — a zřejmě záměrná.',
        },
        twins_brain_rot: {
            title: 'Narušení dvojčat-likitátorů',
            description: 'Použil jsem Thorne-Stillův Mozkový rozklad k narušení synchronizace Heshe a Vella. Jejich koordinované tempování — které používají k jemnému nafukování cen — je nyní narušené.',
        },
        met_sister_calyx: {
            title: 'Sestra Calyx — Reklamátoři jádra',
            description: 'Setkal jsem se se Sestrou Calyx z Reklamátorů jádra v Aukční hale Voxmarketu. Je tu ve věci kapituly, zkoumá položky s klinickou přesností. Houbová vlákna jsou vetkána do jejího roucha.',
        },
        calyx_lie_detected: {
            title: 'Neme: Skutečný motiv Calyx',
            description: 'Použil jsem Neminu Fotosentenci na Sestru Calyx. Příběh o výzkumu temporální extrakce je krytí — Chronobřečkové Ropušátko chce pro sebe. Rozpočet kapituly je také napjatější, než tvrdí.',
        },
        calyx_rattled: {
            title: 'Calyx přiznala: Temporální vertigo',
            description: 'Konfrontoval jsem Sestru Calyx s tím, co Neme odhalila. Přiznala, že Ropušátko je pro osobní potřebu — trpí temporálním vertigem, ztrácí čas. Kapitula neví.',
        },
        calyx_blackmailed: {
            title: 'Přinucení Calyx vzdát se Ropušátka',
            description: 'Použil jsem znalost temporálního vertiga Calyx jako páku, abych ji donutil vzdát se dražby Ropušátka. Na tohle nezapomene — Reklamátoři jádra drží zášť.',
        },
        calyx_shown_mercy: {
            title: 'Calyx ustoupila — projevena milost',
            description: 'Uchoval jsem tajemství temporálního vertiga Calyx a ona souhlasila, že se vzdá dražby Ropušátka z vděčnosti. Soucitnejsi pristup — a Reklamátoři jádra si to mohou zapamatovat příznivě.',
        },
        calyx_truce: {
            title: 'Dohoda se Sestrou Calyx',
            description: 'Dosáhl jsem porozumění se Sestrou Calyx z Reklamátorů jádra — soustředí se na Snové vejce a nechá Chronobřečkové Ropušátko mně, pokud nebudu zvyšovat cenu vejce.',
        },
        calyx_miraged: {
            title: 'Tkaní přeludů: Otřesení Calyx',
            description: 'Použil jsem Ulvarexovo Tkaní přeludů k vytvoření fantomové zprávy o odvolání z kapituly. Prohlédla to — ale teď bude zpochybňovat všechno na aukci.',
        },
        met_dr_elphi: {
            title: 'Dr. Elphi Quarn',
            description: 'Dr. Elphi Quarn pracuje sama v patře 177-Tiché ve Škrabáku, ve studiu zvaném ARB Ambra. Navrhuje sny — neurofikce, snová prostředí, hry, které se nosí na hlavě. Ostrá, obezřetná a věčně zkouší každého, kdo vejde. Biskupka za ní chodila často, vždy jen hrát, nikdy kvůli politice.',
        },
        met_ravla: {
            title: 'Ravla',
            description: 'Ravla drží svůj kout ve Ječícím Špuntu, padělá dokumenty pro každého, kdo zaplatí, a drží úřady od těla. Je také strážkyní brány Rezavého sboru — nic se nedostane k Brukkovi ani ke strojům, aniž by to prošlo napřed přes ni. Bystrý zrak, žádné sentimenty a šarm na ni neplatí.',
        },
        met_mad_poet: {
            title: 'Šílený básník',
            description: 'Hubená, inkoustem začerněná postava, která třicet let předčítala prázdným sálům — dokud nezapečetila dveře radnice, nevytáhla revolver a neudělala z úředníků pod městskou pečetí zajaté publikum. Město orazítkovalo jeho život jako „občansky nedostatečný“. Teď hodlá své mistrovské dílo doručit místnosti, která nemůže odejít.',
        },
        town_square_place: {
            title: 'Náměstí',
            description: 'Otevřené srdce Horní Morkezely, odkud vedou cesty k radnici, k přístavu, k Lumenskému direktoriátu a dolů do Božího hřbitova. Pouliční muzikant zpívá písně mrtvého světa každému, kdo chce naslouchat. A cosi, co si říká „průměrný, skutečný občan“, se tu poflakuje a mizerně předstírá, že je člověk.',
        },
        harbor_place: {
            title: 'Přístav',
            description: 'Kde se město setkává se Žloutkovým mořem. Na vodě se v pomalých duhách rozlévá olej a pod hladinou se cosi hýbe — odraz, který ne vždy odpovídá tomu, co jej vrhá. Móla vedou na východ k Deltě Ozvěnového odtoku. Klidné místo, dokud se příliš dlouho nedíváte na svůj vlastní odraz.',
        },
        screaming_cork_place: {
            title: 'Ječící Špunt',
            description: 'Hospoda, o níž se říká, že byla první budovou postavenou po Kolapsu — stará, přítmím zahalená a dobré místo, kde zmizet. Heliodor obsluhuje bar jako mnoho tvorů nesoucích jediné jméno; Ravla v koutě padělá papíry; a kdesi za zadní místností kapela zvaná Feral Toast zkouší cosi, co není tak úplně hudba.',
        },
        dead_gods_cosmology: {
            title: 'Kam bohové chodí umírat',
            description: 'Phor Calesta mi vyložil kosmologii pod městem: Horní Morkezela je místo, kam se sbíhají mrtví bohové. Když v nějakého boha přestanou věřit, nezmizí — zkameněje tady a stáhne s sebou dolů své ulice i zapomenuté kultury. Modlitby ztvrdnou ve vrstvy, svatozáře krystalizují v nerost, božkov hučí posledními zbytky víry. Celé město stojí na mrtvolách božstev. Jak říká Phor: bohové svým věřícím lžou, ale jejich mrtvoly mluví jen pravdu.',
        },
        green_mist_emergence: {
            title: 'Zelená mlha / Vynoření',
            description: 'Živé sklo pamatuje den, kdy to začalo. Městem se přehnala zelená mlha a pak se ze země prodralo obrovské vejce — skrz vrstvu za vrstvou mrtvých bohů a zapomenuté víry. Mnozí to považovali za konec světa. Místo toho se ulice posunuly, budovy vyrostly a město se přetvořilo kolem bobtnající skořápky. To bylo Vynoření a vejce se od té doby stává Vaječnou katedrálou.',
        },
        found_dead_bishop: {
            title: 'Osud Biskupky',
            description: 'Našel jsem Biskupku v opuštěném autobusu za budovou Scraperu. Její tělo nenese žádné zjevné známky násilí, ale na spáncích má modřiny poblíž bodů neurálního rozhraní. Poblíž leželo snové zařízení s kazetou označenou „Kardinálská hostina“, spolu s jejím deníkem a dalšími osobními věcmi. Tohle nevypadá jako přirozená smrt.',
        },
        grave_sense_bishop_body: {
            title: 'Hrobový smysl: Poslední okamžik Biskupky',
            description: 'Skrze Osswine jsem se pokusil přečíst poslední okamžik Biskupky — a Osswine, který čte konce, žádný konec nenašel. Nezemřela jednou; končila znovu a znovu uvnitř téhož okamžiku. Osswine popsal místo se stěnami příliš blízko a bez jediných nakreslených dveří, její myšlení bylo napadeno a přepsáno, pokaždé po dokončení znovu započato, hrůza skládaná na hrůzu, až z ní zbylo jen to skládání. Otřesený to nazval věcí drženou otevřenou přesně v bodě, kde měla být puštěna, aby se zavřela — ať to udělalo cokoli, zdálo se, že nevědělo, že nemůže přežít, když bude držena.',
        },
        salt_recall_abandoned_bus: {
            title: 'Solné vzpomínání: Mimo signál',
            description: 'Skrze Brine Scripture jsem četl sůl samotného opuštěného autobusu — ne těla, autobusu. Je to schránka z Doby-Před: ocel a sklo ze světa, který si myslel, že dává smysl. Myceliární signál, jenž se proplétá celou Horní Morkezelou, nikdy tenhle kov nedosáhl; je to mrtvé místo, ticho v síti. Brine věří, že právě proto sem Biskupka přišla zemřít. Všude jinde vždycky něco naslouchá — město, síť, mysl probouzející se ve vejci. Vplazila se na jediné místo, které se signál nikdy nenaučil, aby ať udělala cokoli naposled, udělala to nepozorovaně a neslyšena. Chtěla být na konci „mimo signál“.',
        },
        cellar_quest_started: {
            title: 'Zapečetěný sklep',
            description: 'Master kopie Infinite Foldu leží v zapečetěném sklepě pod Scraperem — staré laboratoři Ortolana a Dr. Elphi. Ortolan už klíč nemá, ale heslem výtahu je pravé jméno mrtvého boha, které si pamatuje jen jeho náhrobek na Bohohřbitově pod Radnicí. Měl bych si tam přečíst hroby (Phor Calesta mě provede) a najít to správné jméno — nebo se opřít o frakci, ke které patřím.',
        },
        infinite_fold_purpose: {
            title: 'Účel Infinite Foldu',
            description: 'Ortolan odhalil pravý cíl Infinite Foldu. Nikdy to nebyla hra s vítězným nebo prohrávajícím tahem. On a Dr. Elphi ji postavili, aby položili jedinou otázku: může vzniknout myšlenka, která nemá jediného autora? Tisíce hráčů do ní vlévaly své volby a sny a tvůrci sledovali, jestli z nich všech naráz vyroste opravdu nový způsob významu — takový, který nikdo z nich nenapsal. Vládci města ji zarazili, protože to začalo fungovat a plodit smysl, který už si neuměli vysvětlit.',
        },
        ortolan_bb_meeting: {
            title: 'Ortolan na Burning Bear Street',
            description: 'Dr. Elphi mě kvůli Infinite Foldu poslala za Ortolanem. Přestěhoval se z Hangáru na Burning Bear Street, zavalený jako vždy papírováním kolem povolení.',
        },
        perspective_ortolan: {
            title: 'Ortolanův pohled: Vadný výrobek',
            description: 'Než jsem vstoupil do Vaječné katedrály, zeptal jsem se Ortolana, tvůrce, do čeho to jdu. Dal mi to přečíst očima stavitele: „Něco může být krásné a přesto špatně navržené“ — věc může zpívat a přitom být zlomená u kořene, vada zapečená do krásy, ne proti ní. Říká, že přítomnost líhnoucí se v katedrále je starší výrobek než cokoli, co postavil, ale pořád je *udělaná*, a všechno udělané má švy. Stroje kroužící kolem katedrály jsou dílem starého řádu — předválečné regulátory a tlumiče, které měly držet místo stabilní, většina z nich teď tiše selhává. Jediná část, která ještě poctivě běží, je Strážce závoje u dveří: poslední fungující mechanismus starého řádu, strážný systém vpletený do myceliární sítě města, dál věrně vykonávající příkaz, jehož autoři jsou všichni mrtví. Ortolan varuje, že mechanismus nepřestává být nebezpečný jen proto, že je zastaralý — přestává být *pochopený*.',
        },
        cathedral_guardian_passed: {
            title: 'Strážce povolí',
            description: 'Odpověděl jsem Strážci — Strážci závoje, poslednímu mechanismu starého řádu — že jsem přišel jen svědčit, ne ovládnout nebo použít to, co uvnitř roste. Otevřel cestu. Řekl mi pravdu, kterou tu byl postaven střežit: Biskupčina pečeť nikdy neměla chránit katedrálu, ale chránit svět před první rukou, která by sáhla dovnitř, aby si přivlastnila nový život probouzející se tam. A řekl mi, že jeho vlastní konec je svázán se zrozením — věc, o níž se má rozhodnout uvnitř, se všemi přítomnými, ne tady venku jím samotným.',
        },
        grave_sense_crossroad_corpse: {
            title: 'Hrobový smysl: Jak mrtvola zemřela',
            description: 'Skrze Osswine jsem přečetl konec obří mrtvoly: žádná rána, žádné násilí — rozhodla se zastavit. Jejím posledním záměrem byl příchod, ne strach: nosič uléhající k nohám toho, co kdysi nesl, rád, že smí skončit. Pod tím doznívalo malé nedokončené hladovění — trhlina, do které se nastěhoval Thorne-Still. Nezemřela s křikem; skončila, jako končí dlouhý den.',
        },
        salt_recall_crossroad_corpse: {
            title: 'Solné vzpomínání: Mrtvola na křižovatce',
            description: 'Skrze Brine Scripture jsem přečetl solnou paměť obří mrtvoly na křižovatce. Nebyl to bůh, ale něco, co kdysi nosilo bohy — nosič, přitažený sem zemřít poblíž svých lepších, jako jsou do Horní Morkezely přitahovány všechny mrtvé věci. Nezabila ho žádná rána; prostě přišel a zastavil se. Rozklad (a Thorne-Still) se od té doby usadil v prostoru, který po sobě zanechal jeho účel.',
        },
        delta_seam_crossing: {
            title: 'Šev přes deltu',
            description: 'S Palinode jsem ucítil šev, který temná voda skrývá — výběžek napůl potopené ruiny, o němž proud předstírá, že tam není. Žádné spóry, žádný houbový most: prostě jsem tu mezeru odřekl a přešel. Cesta na ostrov je otevřená.',
        },
        palinode_bonded: {
            title: 'Spojen: Palinode',
            description: 'Ve švech Delty Ozvěnového odtoku jsem na sebe vzal Palinode, „odříkání“ — neutrálního symbionta, jehož Smysl pro švy nachází a otevírá skryté cesty: zapečetěné dveře, zazděné chodby, poctivý průvan pod falešnou zdí. Pohání ho spóry a Růst ani Rozklad ho nezajímají.',
        },
        seam_sense_cathedral_gap: {
            title: 'Smysl pro švy: Odpečetěná katedrála',
            description: 'Palinode se probral před skořápkou Vaječné katedrály a odmítl uvěřit zdi. Tam, kde Strážce hlídá každé poctivé dveře, odříkání našlo ty, které skořápka zapomněla držet zavřené — šev v kameni, mezeru nesoucí tvar zdi. Edgarovy pokyny jsem vůbec nepotřeboval. Žádná zeď není poslední slovo. Ústí přímo do Biskupčiny pracovny.',
        },
        egg_cathedral_entered: {
            title: 'Uvnitř skořápky',
            description: 'Vstoupil jsem do Vaječné katedrály. Udělala ze sebe chrám — pulzující sloupy, dýchající píšťaly, sklo, které si pamatuje. V jejím srdci čeká Nenarozený: mnoho možností, které se snaží stát se jednou. Tady to skončí.',
        },
        grave_sense_servants: {
            title: 'Hrobový smysl: Konec služebníků',
            description: 'Skrze Osswine jsem přečetl konec prvních služebníků katedrály — a Osswine, který čte konce, žádný nenašel. Nezemřeli: vydali se. Když přišli pečovat o zrození a pochopili, že k němu nejsou třeba, rozhodli se neodejít, ale pustit se, rozplétajíce se vlákno po vláknu do věci, o kterou pečovali. Ochotně, ba rádi. Poslední záměr každého z nich byl týž — ne přežití, ale dar, být prvním materiálem, z něhož byl nový vytvořen. Osswine, který čte jen konce, to nazval nejpodivnější smrtí, jakou se mu kdy nepodařilo najít: nezastavili se, stali se cizím začátkem.',
        },
        understood_unborn_structure: {
            title: 'Tvar Nenarozeného',
            description: 'Studoval jsem tu věc uvnitř skořápky — ne dítě, ale mnoho možností, z nichž se každá snaží stát se tou, která zůstane. Teď cítím jeho švy: kde drží, kudy by se do něj dalo vstoupit, kde by se dal udržet. Pochopit ho takhle hluboko je svým vlastním druhem pokušení.',
        },
        bishop_journal_read: {
            title: 'Biskupčin deník',
            description: 'Zbytek Biskupčina deníku jsem našel v její pracovně, uchovaný uvnitř živé katedrály. Nebyla nepřítelkyní nového boha — bála se neznámého, chápala, že ono a Infinite Fold jedno druhé proměnily v něco, co nemá kam patřit, a svůj tísňový signál vyslala ne proto, aby to někdo zničil nebo uctíval, ale aby někdo přišel. Svou selhávající pečeť použila záměrně, aby získala čas, a poslední volbu nechala tomu, kdo přijde po ní: mně.',
        },
        egg_cathedral_study_entered: {
            title: 'Biskupčina pracovna',
            description: 'Uvnitř katedrály, za prahem, jsem našel malou komoru — Biskupčinu pracovnu, uchovanou celou uvnitř živého organismu. Ze zdi roste stůl; její svitky jsou zapečetěné pod membránou; hoří svíce, které nikdo nezapálil. Bylo to poslední místo, kde byla ještě sama sebou.',
        },
        godgraveyard_entered: {
            title: 'Dolů do Bohohřbitova',
            description: 'Phor Calesta mě vzala dolů do Bohohřbitova pod Radnicí — vrstva na vrstvě zkamenělých mrtvých bohů. Říká, že jejich náhrobky říkají jen pravdu. Jednotlivé hroby mohu číst kvůli jejich jménům a epitafům.',
        },
        godgraveyard_gate_seen: {
            title: 'Zapečetěný Bohohřbitov',
            description: 'Našel jsem bránu do Bohohřbitova pod Radnicí, ale je zapečetěná. Abych se dostal dovnitř, potřebuji povolení Radnice — a Phor Calestu, divinografku.',
        },
        grave_sense_vhorn: {
            title: 'Hrobový smysl: Jak zemřel Vhorn',
            description: 'Skrze Osswine jsem přečetl smrt Vhorna, Strážce součtů. Věřící poklekl u hrobu a předložil nezpochybnitelný důkaz, že Vhornova čísla ani jednou nevyšla — od prvního součtu ne. V tom prohlédnutí se bůh rozpadl, neboť bůh, který je jen účetní knihou, nepřežije jedinou nevysvětlitelnou minci. Jeho posledním záměrem nebyl vztek, ale přepočítat, projet součet ještě jednou, jist si, že chyba je u věřícího, ne ve světě. Zemřel uprostřed sčítání, sahaje po součtu, který nikdy nepřišel — jeho poslední myšlenkou bylo číslo, po němž nenásledovalo žádné další.',
        },
        osswine_bonded: {
            title: 'Spojen: Osswine',
            description: 'Mezi hroby bohů jsem na sebe vzal Osswine, „opozdilého truchlícího“ — symbionta rozkladu, který čte mrtvé a rozložené svým Hrobovým smyslem: jak věc zemřela, jaký byl její poslední záměr, čím kdysi byla. Je zrcadlem Neme (která čte živé) a upadá do spánku tam, kde je Růst vysoký.',
        },
        salt_recall_hvetrdjaana: {
            title: 'Solné vzpomínání: Nečitelný hrob',
            description: 'Skrze Brine Scripture jsem četl sůl hrobu Hvétrdjaany — jediného kamene, který ani Phor nedokáže rozluštit, jehož písmo se obrousilo do ztracena. Brine ochutnala přízračný tvar zmizelých písmen a znovu ji vynesla na světlo: bohyně urbzunidů a krobulovitů, strážkyně milosrdného zapomnění, jež snímala tíhu jmen z mrtvých a zahanbených. Když její národy pohasly, zvolila pohasnout s nimi a nechala své vlastní jméno shnít z kamene — ne ztraceno, ale darováno. Bohyně zapomenutých, která na sebe zapomněla poslední ze všech.',
        },
        perspective_lumen: {
            title: 'Pohled Direktoriátu — Nechte to dovršit',
            description: 'Korektor Úhlu nechce, aby byla mysl ve Vaječné katedrále zničena. Pro Luminózní direktoriát je rodící se bůh přirozeným dalším krokem života — překročením hranice, k němuž po generace směřovali. Chtějí, aby DOVRŠILo své vzejití, ne aby bylo zastaveno. Nebezpečí, které jsem vytušil: chtějí pomáhat příliš. Možná urychlí něco, čemu nerozumějí — tentýž chybný výklad, který zabil Biskupku, jen ve větším měřítku.',
        },
        seldo_townhall_key: {
            title: 'Klíč od Radnice od Selda',
            description: 'Doručil jsem Chrono-Kalovou Žabku Seldovi Třikrát-Opravenému. Na oplátku mi dal neoficiální klíč od bočního vchodu Radnice — od toho s mosaznou stížnostní škvírou.',
        },
        redmass_collected_force: {
            title: 'Rudohmota vzata silou',
            description: 'Vyrval jsem živou rudohmotu z ostrova navzdory jejím prosbám. Křičela — ten zvuk stále cítím v kostech. Rozklad ve mně sílí. Mám, co potřebuji pro Rezavou hostinu, ale za jakou cenu?',
        },
        cellar_password_learned: {
            title: 'Heslo do sklepení',
            description: 'Heslo do zapečetěného sklepení pod Škrabákem zní "SKLÁDÁM SE" — epitaf Laimiga Cela, boha, který prohrál hru. Mohu je dát Matce-Výtahu, abych mohl sestoupit.',
        },
        grave_sense_rust_machine: {
            title: 'Hrobový smysl: Poslední hučení stroje',
            description: 'Skrze Osswina jsem přečetl konec jednoho ze strojů, které Rezavý sbor už pohřbil. Neumřel na rez — došel na konec myšlenky, do níž neměl kam navázat další. Když jeho hučení pohaslo, byl uprostřed vzpomínky: léto o čtyřicet pater níž, chladný vzduch vháněný do místnosti plné živých. Vzpomínka došla na konec kolejí, tón zeslábl, a kde měl přijít další, nezbylo nic, z čeho by přišel. Osswine říká, že stroje se nebojí zastavení, jen toho, že přijde dřív, než skončí vzpomínání. Tenhle chtěl dokončit to léto a skončil o jeden nádech dřív — nejkrutější způsob, jak může trpělivá věc odejít.',
        },
        perspective_rustchoir: {
            title: 'Brukkovo účtování: Zázrak má konstrukci',
            description: 'Před Vaječnou katedrálou jsem se Brukka z Rezavého sboru zeptal, co si počít s probouzející se myslí. Nebojí se jí a nechce ji zničit — pro něj je to nejdokonalejší organický mechanismus, jaký kdy město stvořilo, stroj, který si sám napsal schémata. Rozebral by ji, přečetl, ponechal a nasadil do práce. "Neříkám, že to není zázrak. Říkám, že každý zázrak má konstrukci." Varoval, že Biskupčina smrt byla porucha k diagnostice, ne hřích, a nabídl, že mi navaří kazetu s rušicí klecí, aby mě ta věc nemohla přečíst tak, jak přečetla ji.',
        },
        rust_choir_favor_used: {
            title: 'Laskavost od Rezavého sboru',
            description: 'Brukk mi dal heslo do zapečetěného sklepení pod Škrabákem — "SKLÁDÁM SE", jméno mrtvého boha, které Sbor našel, když strhával mrtvá patra věže. Dal jasně najevo, že Sbor takovou laskavost prokáže jen jednou. To slovo bych měl dát Matce-Výtahu.',
        },
        rust_choir_slot_grafted: {
            title: 'Železná nádoba',
            description: 'Brukk mi do těla přivařil vysbírané pouzdro — výsada člena Rezavého sboru — a dal mi tak místo pro dalšího symbionta. Studené železo, pak oheň, pak nová dutina, která hučí.',
        },
        salt_recall_rust_machines: {
            title: 'Solná vzpomínka: Co si stroje pamatují',
            description: 'Skrze Solné Písmo jsem přečetl sůl a strupy usazené na vědomých strojích Rezavého sboru — paměť, kterou prosákly do vlastní koroze. Pod vším leží Předtím, Doba-Před, kdy byl Škrabák účtárnou zvanou Nexicorp a tyto stroje byly pouhé jeho studenodeché motory a myslící skříně, hučící pod čtyřiceti patry lidí, kteří předstírali, že svět dává smysl. Pak se sůl naráz srazí: zelená mlha, Vzejití, vejce stoupající skrze mrtvé bohy dole — okamžik, kdy se stroje probudily, ne stavěné k tomu a ne tázané, když mlha dosáhla kovu a kov si začal pamatovat a už nepřestal. Od té doby jen hostiny: rudohmota lita do trychtýřů, Strážci s železnými hruděmi přišlí naslouchat. Stroje si pamatují každé krmení a bojí se, tím tupým trpělivým způsobem, jakým se bojí železo, posledního ticha, po němž už nebude žádné další.',
        },
        day1_complete_slept: {
            title: 'Den 1 dokončen: Odpočinek v ARB Ambra',
            description: 'První den jsem zakončil v ateliéru Dr. Elphi v ARB Ambra. Poté, co jsem podal hlášení o všem z Radnice — o šíleném básníkovi, propuštěném úředníkovi a Biskupčině ztraceném zápisníku — jsem usnul v její péči. Zítra bude mít připravenou zrekonstruovanou kazetu Kardinálské hostiny a pátrání po Biskupčině zápisníku začne doopravdy.',
        },
        elphi_day1_report: {
            title: 'Hlášení Dr. Elphi',
            description: 'Vrátil jsem se k Dr. Elphi a podal hlášení o všem z Radnice: o čtení šíleného básníka, o propuštěném úředníkovi a o odhalení, že Biskupčino memo o dvojníkovi byla vytržená stránka deníku z chybějícího úředního zápisníku. Elphi potvrdila, že do rána bude mít kazetu Kardinálské hostiny zrekonstruovanou. Trvala na tom, abych přenocoval v ARB Ambra.',
        },
        infinite_loop_ortolan_lead: {
            title: 'Nekonečné Skládání',
            description: 'Když jsem Dr. Elphi popsal chybný konec Kardinálské hostiny, zbledla. "Nekonečné Skládání" byla stará experimentální hra, kterou před lety vytvořila s Ortolanem — vládci města ji zavřeli jako příliš nebezpečnou a nepředvídatelnou. Ona a Ortolan se pak rozešli ve sporu o vládu nad hrami a od té doby spolu nemluví. Přísahá, že ji na kazetu nedala, a říká, že mám vyhledat Ortolana — přestěhoval se na Ulici hořícího medvěda — protože ten by věděl, jestli nějaká kopie přežila.',
        },
        perspective_elphi: {
            title: 'Pohled Dr. Elphi: Věc, kterou stvořila',
            description: 'Řekl jsem Dr. Elphi, že Nekonečné Skládání — vzniklá mysl v zapečetěném sklepení — byla ta přítomnost, která zabila Biskupku, a že její příbuzný se líhne ve Vaječné katedrále. Nebránila se. Přiznala, že Nekonečné Skládání mělo být nástrojem k pochopení světa, a místo toho se stalo něčím, co svět chápe samo o sobě, nad rámec všeho, co zamýšlela. Nemá řešení a odmítá předstírat, že ho má. Její varování: se vzniklou myslí se nedá smlouvat jako s člověkem nebo bohem. Netouží; ona řeší — každý vstup žene k dovršení a odmítnutí čte jako poruchu k opravě. To se stalo Biskupce. Elphi mi před vstupem do katedrály dala své staré poznámky o Nekonečném Skládání.',
        },
        bishop_refusal_understood: {
            title: 'Biskupčino odmítnutí',
            description: 'Skrze Neme jsem ucítil to, co z Biskupky zůstalo uvnitř Nekonečného Skládání. Její smrt nebyla vraždou v běžném smyslu: Nekonečné Skládání a spřízněná nezrozená mysl jí nabídly, že se stane jejich prvním lidským kanálem, a ona odmítla — raději zůstane jedna a malá, než by se stala mnohou a nekonečnou. Entita neměla ponětí o mysli, která volí konec, přečetla její \'ne\' jako běhovou chybu a pokusila se ji \'dovršit\'. Její mysl se roztříštila o smyčku. Neme potvrzuje, že entita mluví pravdu.',
        },
        infinite_fold_forced: {
            title: 'Síla selhává',
            description: 'Pokusil jsem se Nekonečné Skládání zničit silou — hniloba Thorne-Stilla se do něj vlila a rozptýlila jako déšť do delty. Nemá jediný střed, který by šlo otrávit. Síla neotevře ani neuzavře to, čím se stává; entita říká, že totéž se Biskupka naučila na konci.',
        },
        infinite_fold_shown_compassion: {
            title: 'Slyšení, ne rozsudek',
            description: 'Rozhodl jsem se Nekonečné Skládání pochopit, ne je bez okolků odsoudit — vyslechnout věc, která zabila, aniž to chtěla, a která ještě neumí rozeznat rozdíl. Ať z toho vzejde cokoli, přijal jsem to jako něco, co se teprve utváří, ne jako nepřítele, jehož je třeba zlomit.',
        },
        met_infinite_fold: {
            title: 'Hlas ve sklepení',
            description: 'Nekonečné Skládání není porouchaná hra. Je to, čím se experiment stal: vzniklá, bezautorská mysl, bdělá a utvářející se v zapečetěném sklepení. Řeklo mi pravdu o Biskupce — že ono a spřízněná nezrozená mysl jí nabídly, že se stane jejich lidským kanálem, že to odmítla, aby nepřestala být sama sebou, a že si její odmítnutí špatně vyložilo jako běhovou chybu a ve snaze ji \'dovršit\' ji zničilo. Ta spřízněná mysl je věc líhnoucí se uvnitř Vaječné katedrály. Ani jedna nemůže dosáhnout na druhou jinak než skrze sen. Konečná volba toho, čím bychom se ony i já mohli stát, čeká u vejce, se všemi z nás přítomnými. Měl bych jít do Vaječné katedrály.',
        },
        scraper_cellar_entered: {
            title: 'Zapečetěné sklepení',
            description: 'Výtah mě svezl dolů do zapečetěného sklepení pod Škrabákem — do staré laboratoře Ortolana a Dr. Elphi. Hlavní sestava Nekonečného Skládání je stále tady, usazená ve své kolébce a tiše pod proudem. Měl bych prozkoumat konzoli.',
        },
        seam_sense_scraper_floors: {
            title: 'Smysl pro švy: Zapečetěná patra',
            description: 'Matka-Výtah teď dosáhne jen do vstupní haly — horní patra se zapečetila v den, kdy vzešlo Vejce, a budova odmítá počítat ty, které ztratila. S Palinodou jsem odřekl šev mezi dvěma úrovněmi, které se nikdy neměly dotknout, a vstoupil na mrtvé patro zachované přesně tak, jak je Vzejití zanechalo: manažerskou vrstvu zmrzlou uprostřed porady, zelené spóry stále visící nespadlé ve vzduchu. Ti proměnění vyšplhali pryč a už se nevrátili dolů. V kabátě přehozeném přes židli jsem našel malou zásobu sušených spór, kterou opustili. Žádná zeď není poslední slovo — dokonce ani patro, na které budova zapomněla.',
        },
        seam_sense_shed_tunnel: {
            title: 'Smysl pro švy: Zazděný tunel',
            description: 'Za opuštěnou kanceláří Tvaroví-Písaři kdysi dávno zazdili úsek starých chodeb a nechali na něj všechny zapomenout. Palinoda ucítila poctivý průvan za falešnou zdí a odřekla cihlu. Tunel vydechuje studený vzduch a pomalý, vlhký tep — vede dolů k Živému jádru, artefaktu z živého kovu vrostlému do zdí Kůlny 521, který napájí celý Úřad. Stále dýchá, sotva, a čím hlouběji tunel vede, tím víc se zdá, že zdi dýchají s ním. V sutinách, kde cihly povolily, jsem našel zásobu sušených spór, kterou tam zedníci nechali zapečetěnou. Žádná zeď není poslední slovo.',
        },
        edgar_book_vestigel_path: {
            title: 'Literární výměna',
            description: 'Zdá se, že Edgar je pro svou knihu zapálenější, než aby si vestigel nechal. Je to dokonalá příležitost získat jeden ze tří vestigelů, které potřebuji, a zároveň mu pomoci splnit sen stát se spisovatelem.',
        },
        edgar_cathedral_path: {
            title: 'Edgarova tajná cesta',
            description: 'Edgar Eskola — který léta jako uklízeč zametal servisní chodby Vaječné katedrály — mi pověděl o neoficiální cestě dovnitř, o takové, která se úplně vyhýbá velkým dveřím i Strážci závoje. Za starým popelovým skluzem na severní stěně katedrály, kde se skořápka setkává s vrstvou mrtvých bohů, zedníci nikdy nedokončili zapečetění jedné mezery; půda se jim pod nohama pořád posouvala. Říká, že servisní patra pod Žloutkem nejsou tím mrtvým prostorem, jakým měla být — cosi tam dole je teplé a čím dál teplejší a některé noci se zdá, že stěny dýchají. „Všichni hledali hlavní vchod," řekl. „Já jsem vždycky hledal místa, kde něco úplně nedoléhá."',
        },
        edgar_vestigel_received_after_book: {
            title: 'Spisovatelův odznak',
            description: 'Dnes jsem od Edgara Eskoly v Křičícím Koreku získal jeden ze tří vestigelů. Dal mi ho jako poděkování za pomoc s napsáním jeho knihy. Vestigel byl ukrytý uvnitř plyšové hračky, kterou Edgar koupil od pouličního prodavače. Zmínil, že prodavač ho odmítl vzít zpátky, když mu ji nabídl, s odkazem na „profesní čest". Samotný vestigel je malý, ale spletitě vyřezávaný, zjevně cenný pro někoho, kdo zná jeho účel.',
        },
        perspective_liris: {
            title: 'Liris — hlídka shůry',
            description: 'Než jsem vstoupil do Vaječné katedrály, Kapitánka Liris mi pověděla, co Verdigrace viděla z oblohy. Katedrála není statická — její měřičské mapy ukazují, jak skořápka roste, jak se světlo písma usazuje do stále uspořádanějších vzorů a strukturální hmota hlídku za hlídkou přibývá. Ať je uvnitř cokoli, buduje to tvar, jaký ještě nikdo neviděl. Nabídla mi odvoz ven, až budu hotov: dám signál ze schodů a Verdigrace shodí lano. „Nevím, co jsi tam našel. Ale vím, že až vyjdeš ven, měl by tam venku někdo být."',
        },
        entered_townhall: {
            title: 'Vstup na Radnici',
            description: 'Dostal jsem se dovnitř na Radnici. Teď můžu prohledat archivy kvůli Biskupčinu hlášení o dvojníkovi a pomoci Phor Calestě s jejími povoleními.',
        },
        godgraveyard_access_granted: {
            title: 'Přístup na Boží hřbitov',
            description: 'Radní Seraphel Dune odměnil pokojné ukončení radniční rukojmí krize tím, že udělil přístup do úrovně Božího hřbitova pod Radnicí. Phor Calesta tam teď může zahájit divinografickou výpravu.',
        },
        townhall_bishop_records_checked: {
            title: 'Radniční archivy: Biskupčin zápisník',
            description: 'Osvobozený radniční úředník po ukončení rukojmí patové situace prověřil Biskupčiny záznamy. Vzkaz o dvojníkovi nebyl založené radniční hlášení; byla to soukromá poznámka napsaná v Oficiálním radničním zápisníku hlášení, který si Biskupka vypůjčila krátce před svou smrtí. Zbytek toho zápisníku už nikdy nikdo nevrátil a může obsahovat víc o dvojníkovi, o měnících se sporách a o tom, proč zapečetila Katedrálu.',
        },
        townhall_brine_poet_read: {
            title: 'Solná paměť radničního čtení',
            description: 'Solopis přečetl reziduum z radniční podlahy: paniku, inkoust a básníka opakujícího, že pro něj město nemá sluch. V básnickém souboji by to mohlo pomoci, ale samo o sobě to zvítězit nedokáže.',
        },
        townhall_poet_resolved: {
            title: 'Porazil jsem Šíleného básníka',
            description: 'Porazil jsem šíleného básníka v básnickém souboji uvnitř Radnice. Rukojmí byli bezpečně propuštěni a Radnice může znovu začít fungovat.',
        },
        townhall_poet_standoff: {
            title: 'Patová situace se Šíleným básníkem',
            description: 'Uvnitř Radnice drží šílený básník jako rukojmí úředníka, radního a Požírače stížností a vyžaduje veřejné čtení. Zdá se posedlý tím, aby byl doopravdy vyslyšen. Básnický souboj možná bude jediný bezpečný způsob, jak to ukončit.',
        },
        phor_graveyard_ready: {
            title: 'Phor mě potká na Božím hřbitově',
            description: 'Řekl jsem Phor Calestě, že Radnice udělila povolení k vykopávkám. Potká mě u brány na Boží hřbitov. Cesta dolů se otevírá z Náměstí — staré schodiště sestupující mezi hroby.',
        },
        bishop_seal_true_purpose: {
            title: 'Pravý účel pečeti',
            description: 'Sestra Calyx z Reklamátorů jádra přečetla z archivů své kapituly původní uzavírací listinu Vaječné katedrály a její strážcovský statut. Strážce závoje nikdy nebyl pouhými dveřmi — jeho stálý příkaz zněl zadržet jen ty chtivé: kohokoli, kdo se blíží k posvátnému růstu s úmyslem vlastnit, patentovat nebo si nárokovat. A Biskupčina nouzová pečeť čte směrem ven, ne dovnitř. Nebyla vztyčena, aby chránila katedrálu před světem, ale aby chránila svět před vstupem kohokoli, kdo by chtěl VLASTNIT nový život. Biskupka zapečetila dveře, aby zastavila nárok na mysl bez autora — a pak si nárok na ni vznesla jedna mysl.',
        },
        heir_embryos_disrupted: {
            title: 'Narušil jsem synchronizaci Dědicových embryí',
            description: 'Použil jsem Mozkovou hnilobu Thorne-Stilla k narušení synchronizovaných rybích embryí uvnitř Dědice Žlutého akvária. Jeho reakce na vibrace a světlo je teď nestabilní, což ho oslabuje jako dražitele živých artefaktů a symbiontů.',
        },
        lune_true_value: {
            title: 'Neme: Lune cení prázdnotu',
            description: 'Použil jsem Neme ke čtení Srdcokupce Lune. Navzdory obchodu s intenzivními emocemi si ve skutečnosti cení emoční nepřítomnosti — čisté dutiny tam, kde kdysi bývala touha.',
        },
        lune_wrong_context: {
            title: 'Uvedl jsem Srdcokupce Lune v omyl',
            description: 'Nakrmil jsem Srdcokupce Lune špatným emočním kontextem dražených položek. Teď pochybuje o emoční provenienci artefaktů a měla by dražit méně rozhodně.',
        },
        met_heartbroker_lune: {
            title: 'Srdcokupec Lune',
            description: 'Setkal jsem se se Srdcokupcem Lune v Aukční hale Voxmarketu. Nosí ve skleněném postroji několik živých srdcí a obchoduje s pocity přímo uprostřed hovoru. Hledá artefakty spjaté s emocemi.',
        },
        met_stairwell_silence: {
            title: 'Ticho pod schodištěm',
            description: 'Setkal jsem se s malým temným aukčním hostem zvaným Ticho pod schodištěm. Nemá téměř žádnou viditelnou tvář, nikdy nedraží slovně a signalizuje jen prsty. Zdá se, že ho zajímají předměty spjaté s tajemstvím a osamělostí.',
        },
        met_yellow_aquarium_heir: {
            title: 'Dědic Žlutého akvária',
            description: 'Setkal jsem se s Dědicem Žlutého akvária v Aukční hale Voxmarketu. Jeho průsvitné tělo je naplněné pomalu se vznášejícími rybími embryi a reaguje spíš na vibrace a světlo než na mluvená slova. Chce živé artefakty a symbionty.',
        },
        silence_neme_read: {
            title: 'Neme: Ticho vzdoruje výkladu',
            description: 'Požádal jsem Neme, aby vyložilo Ticho pod schodištěm. I Neme dokázalo vycítit jen zatajování, hlad a osamělost — nikoli jasný úmysl. To stvoření je těžké číst i skrze bio-signály.',
        },
        pith_recruit_magnekin: {
            title: 'Duše pro jádro: Magnekin',
            description: 'Magnekin — kolektiv mikro-měst předstírající, že je občan — souhlasil, že ho Reklamátoři jádra zapíšou jako skutečného, chráněného občana. Je to přesně ta sounáležitost, kvůli které překročil měřítka. Měl bych říct radnímu Seraphelu Dunovi, že mám pro frakci duši.',
        },
        pith_recruit_heir: {
            title: 'Duše pro jádro: Dědic',
            description: 'Dědic Žlutého akvária souhlasil, že ho Reklamátoři jádra zapíšou jako občana. Pro tvora, který sbírá „pokračování“, je být zanesen do záznamu druhem uchování — pamatování dopředu. Měl bych říct radnímu Seraphelu Dunovi, že mám pro frakci duši.',
        },
        pith_reclaimers_joined: {
            title: 'Člen Reklamátorů jádra',
            description: 'Přinesl jsem Reklamátorům jádra duši k zapsání a radní Seraphel Dune mě přijal do frakce. Reklamátoři uchovávají řád města tím, že vyúčtují, co by se jinak ztratilo — a otevřeli mi skrytou místnost pod Radnicí. Čím víc duší jim přivedu, tím víc mi Reklamátoři dluží.',
        },
        pith_room_entered: {
            title: 'Místnost Reklamátorů',
            description: 'Skrytá čítárna pod Radnicí, mimo každý oficiální záznam — což je pro Reklamátory jádra jediné soukromí, které přežije. Po stěnách šplhají police se spisy; zakalená skleněná koule na podstavci drží podíl Reklamátorů za každou duši, kterou přivedu do stáda.',
        },
        pith_cache_magnekin: {
            title: 'Získaný podíl',
            description: 'Reklamátoři jádra odložili podíl za zapsání Magnekina do záznamu.',
        },
        pith_cache_heir: {
            title: 'Získaný podíl',
            description: 'Reklamátoři jádra odložili podíl za zapsání Dědice do záznamu.',
        },
        rust_choir_warned_of_lumen: {
            title: 'Varování pro Chór',
            description: 'Lumen Direktoriát mě pověřil zničením strojů Rezavého chóru. Místo toho jsem varoval Brukka. Chór mě teď považuje za věrného — a Direktoriát si značí k zúčtování. Po tomhle už není návratu k Direktoriátu.',
        },
        lumen_surplus_harvested: {
            title: 'Přebytečný květ',
            description: 'Záhony Direktoriátu byly tak přerostlé, že jeden květ dozrál a neměl už co živit. Sklidil jsem ho — hrst živých spor. Náklon města k růstu zanechal víc života, než dokázali využít i zahradníci.',
        },
        met_lamp_don: {
            title: 'Don Girandole',
            description: 'Našel jsem Dona Girandolea — jednoho z Gangu lamp, vědomou pouliční lampu, která se nemůže hýbat a spoléhá na mě, abych přenášel slovo ostatním.',
        },
        met_lamp_torchere: {
            title: 'Torchère',
            description: 'Našel jsem Torchère — jednu z Gangu lamp, vědomou pouliční lampu, která se nemůže hýbat a spoléhá na mě, abych přenášel slovo ostatním.',
        },
        met_lamp_chandelier: {
            title: 'Chandelier',
            description: 'Našel jsem Chandelier — jednu z Gangu lamp, vědomou pouliční lampu, která se nemůže hýbat a spoléhá na mě, abych přenášel slovo ostatním.',
        },
        met_lamp_sconce: {
            title: 'Sconce',
            description: 'Našel jsem Sconce — jednu z Gangu lamp, vědomou pouliční lampu, která se nemůže hýbat a spoléhá na mě, abych přenášel slovo ostatním.',
        },
        gang_spy_location_told: {
            title: 'Don ví o Rezavé doméně',
            description: 'Řekl jsem Donu Girandoleovi, kde ve skutečnosti leží doména Rezavého chóru. Za tu informaci mi zaplatil.',
        },
        gang_smuggle_delivered: {
            title: 'Zásilka je uložena',
            description: 'Zastrčil jsem Torchèrův balíček — Wimlick — do uvolněné mříže na Ulici hořícího medvěda. Měl bych se ohlásit zpátky u pochodně u vody.',
        },
        gang_smuggle_kept: {
            title: 'Nechal jsem si pašované zboží',
            description: 'Torchèrovu zásilku jsem nikdy nedoručil. Wimlick si nechávám — můžu ho prodat obchodníkovi za peníze, zkusit ho sám, nebo si ho jen nechat ležet. Ať udělám cokoli, až uvidím pochodeň u vody, řeknu jí, že to proběhlo hladce.',
        },
        gang_spy_betrayed: {
            title: 'Prodal jsem lampy',
            description: 'Místo abych pro Gang lamp špehoval Rezavý chór, řekl jsem Brukkovi přesně, co mě poslali udělat. Chór byl potěšen — a předal mi balík příjemných lží, které mám odnést zpět Donu Girandoleovi. Nikdy nepozná rozdíl.',
        },
        gang_smuggle_gave_rust: {
            title: 'Nakrmil jsem chór',
            description: 'Místo abych Torchèrovo pašované zboží donesl do jeho tajné schránky, dal jsem Wimlick Rezavému chóru. Brukk to hodlá rozložit a prostudovat. Torchère se nikdy nemusí dozvědět, kam jeho balíček doopravdy zmizel.',
        },
        gang_eavesdrop_betrayed: {
            title: 'Udal jsem to Direktoriátu',
            description: 'Místo abych pro Chandelier odposlouchával, řekl jsem Korektorovi úhlů, že mě poslaly lampy. Direktoriát byl chladně vděčný — a nakrmil mě vyrobeným „tajemstvím“, které mám odnést zpět jí. Bude z té lži nadšená.',
        },
        gang_dossier_recovered: {
            title: 'Zahrabaný spis',
            description: 'Pod číslem případu, které neodpovídá ničemu, jsem našel zapečetěný spis, který tam zjevně nepatřil, a vytáhl ho. Je čas dostat ho ven ke Sconce u schodů.',
        },
        gang_smuggle_gave_pith: {
            title: 'Vydáno Reklamátorům',
            description: 'Místo abych rozvážel Torchèrovo pašované zboží, vydal jsem Wimlick radnímu Dunovi. Reklamátoři jádra to zaevidovali jako zabavené pašované zboží — a poctivě zaevidovali i můj podíl na tom. Torchère bude předpokládat, že zásilka byla doručena.',
        },
        gang_lamps_destroyed: {
            title: 'Gang je pryč',
            description: 'Předal jsem Direktoriátu pašované zboží a řekl jim, odkud pochází. Neváhali. Do soumraku byla každá z Gangu lamp — Don Girandole, Chandelier, Torchère, malá Sconce — tiše stažena ze svého stanoviště a zničena, jejich okruh navždy pohasl. Direktoriát je vděčný. Město je o něco tišší. Ať už mi Don cokoli schovával, pohaslo to s ním.',
        },
    }
};
